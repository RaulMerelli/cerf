# Contributing to CE Runtime Foundation

## Low-quality contributions are **forbidden**

A low-quality contribution might be one of these:

- It damages the code quality or the emulator quality, suggesting that the author doesn't understand programming principles or doesn't want to respect those and the project rules
- It leads to unhealthy communication patterns with a reviewer (e.g., psychological manipulation, etc.) and endless review cycles.
- The author doesn't understand the code he wrote, doesn't know/use what a code editor is (e.g., VS Code), doesn't know how to use GitHub or Git itself. A general understanding is absent.

Of course, this is all subjective - you may have a worse level of understanding or even a better one than the author of the project. We are talking specifically about unhealthy attempts to contribute to the project. It's fine when you make a mistake due to a lower level of experience. It's bad when you vibe code and can't explain the output.

> [!CAUTION]
> **You will be banned from all project resources** if you are going to attempt this.

If you are feeling an itch to vibe code an enhancement to the emulator that suddenly appears to be working, you can create a pull request as a draft and leave it there with explicit text that it is done badly and doesn't deserve a review. Maybe someone will decide one day to pick it up, take your code, maybe co-author you, and finish your feature. But do NOT attempt to smuggle this as a proper pull request and lie about your understanding. This is easy to detect, and you won't be happy to get a ban. 
