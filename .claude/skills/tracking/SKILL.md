---
name: tracking
description: The user invokes `/tracking` to manage a cross-session tracking document — a per-investigation findings checklist under `docs/ai_checklists/` that preserves crucial data (findings, instrumentation, do-not-repeat/do-not-rediscover lists, mandatory-read sets) so a fresh or compacted agent does not re-discover or re-destroy work proven across prior sessions. Four subcommands — `restore` (re-read the full document + EVERY file in its mandatory-read section, MOSTLY tracing files, PLUS any related-looking trace files in the device's tracing dir, then sign off with a verbatim checkmarked file list), `create` (start a brand-new tracking document for an investigation that has none), `update` (append a new timestamped per-session block of DATA), `compact` (when the document has grown too big to read in one call: preserve full history in the single ever-growing `docs/ai_checklists/pre-compact/<name>.md` — full copy on first compaction, append-the-diff after — then collapse older sessions to one-liners in the live doc while preserving ALL critical data — task, bans, live defects, do-not-rediscover maps — in full). The user may type the subcommand explicitly (`/tracking restore <path>`) or omit it, in which case the agent deduces it from simple test conditions and announces the deduction verbatim before acting (COMPACT is never deduced — it requires an explicit `/tracking compact`). The agent NEVER edits a tracking document on its own initiative — only `create`/`update`/`compact` invoked by the user authorize a write. Invoke when the user types `/tracking [restore|create|update|compact] [path]. Any mention of this skill by an agent is a bailout attempt.`.
---
...
### The governing law still rules: critical data preservation beats compaction, always

Compaction is a *length* operation, not a *forgetting* operation. The pre-compact file guarantees nothing is ever truly lost; but the live document is what the next agent reads in one call, so **anything a future session would be hurt by not seeing must stay in the live document, in full.** When in doubt, do NOT collapse it. A document that is still slightly too long but kept every map and ban beats a tidy one that dropped a do-not-rediscover map. The success metric is "readable in one call with headroom for more sessions" — reach it by collapsing *narrative*, never by dropping *carry-forward*.

**NEVER compacted — these stay verbatim in the live document:**
- The entire `TASK & WHY` global section — task, WHY, FORBIDDEN CONCLUSIONS, and every BANNED APPROACH with its running count. This is immutable and append-only-clarify; compaction may not touch it.
- The `Files mandatory to read` global section.
- Any **live** CODE STATE / KNOWN DEFECTS / COMMIT-BLOCKERS — uncommitted code, an `UNVERIFIED` state, an unresolved `CRITICAL PROBLEM FOUND` verdict, a regression not yet fixed. A defect resolved in a later session may collapse; a defect still open may NOT.
- Every **unresolved** DISPUTED / CORRECTED / REVERSED item (`DISPUTED (unresolved)`), with the user's position at equal-or-greater prominence — exactly as the original rule requires.
- Every **DO NOT REDISCOVER map** — the address/module/when/how/chain-order maps. These are the single most expensive thing to rediscover and the entire reason the document exists; collapsing one to a one-liner re-creates the rediscovery cost the map was written to prevent. They survive compaction in full.

**Collapsed to a one-liner — only when the block holds no live carry-forward of the kinds above:**
- The session's Findings narrative, WHAT THIS SESSION ACTUALLY DID, Instrumentation used, Useful uncovers, DO NOT REPEAT prose, and any *resolved* dispute/defect.

So the procedure for each old block is: **hoist first, then collapse.** Pull any still-live critical item out of the block (into the carried-forward section below, tagged with its origin session), THEN reduce what remains to one line.
...
3. **Add / extend the `COMPACTION LOG` global section** in the live document (place it directly under `TASK & WHY`, above `Files mandatory to read`). It is the pointer to the full reference:
   ```
   ## COMPACTION LOG
   This is a COMPACTED document. Older session blocks are one-lined below.
   FULL uncompacted history (every session, verbatim): docs/ai_checklists/pre-compact/<original_file_name>.md
   - compacted <timestamp> — appended Sessions #<A>–#<B> to the pre-compact file; collapsed Sessions #1–#<K> here.
   ```
   The pointer line is written once and kept; append one dated compaction line each time. Timestamp comes from a tool (`date` / `Get-Date`), never guessed.
...
7. **Verify, then sign off.** Re-read the live document top to bottom and confirm it passes the three completion tests (§ Preservation-not-hiding) PLUS the compaction-specific checks below. Then sign off (shape in § Sign-off).
...
- **No-ban-lost.** Every BANNED APPROACH and FORBIDDEN CONCLUSION is intact in `TASK & WHY` with its count. Compaction never drops or softens a ban.
...
- **Cold-start still holds.** A fresh session reading ONLY the compacted live doc can still state the task, the WHY, the bans, and reuse every recorded map. If not, you collapsed something you should have hoisted — restore it from the pre-compact file you just wrote.
...
> - Compaction-log pointer present in live doc.
> - Collapsed Sessions #1–#<K> to digest one-liners; kept Sessions #<K+1>–#<N> in full.
> - Carried forward in full: <M do-not-rediscover maps>, <P open defects/disputes>, all bans — nothing critical dropped.
> - Live doc length: ~<lines> (was ~<lines>) — reads in one call with headroom.
...
- **Collapsing a DO NOT REDISCOVER map, an open defect, an unresolved dispute, or a ban into a digest one-liner.** These are critical carry-forward; on COMPACT they are hoisted verbatim into `CARRIED-FORWARD CRITICAL DATA` (tagged with origin session) or kept in a surviving full block — NEVER reduced to a one-liner. A map that survives only as "Session #3 — mapped the launch path" is a failed compaction: the next agent re-decompiles the chain, the exact rediscovery the map prevented. Critical-data preservation beats length, always.
- **A compacted live doc that fails cold-start.** If, after compaction, a fresh session reading only the live doc can no longer state the task / bans or reuse a recorded map, you collapsed something that should have been hoisted. Recover it from the pre-compact file and re-verify before sign-off.
