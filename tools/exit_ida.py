...
def main():
    ...
    for hwnd, pid, title in ida_windows:
        ...
        if wait_pid_exit(pid, timeout=60.0):
            print(f"  Exited.")
        else:
            print(f"  WARNING: PID {pid} still running after 60s")

    # Final check
    time.sleep(0.5)
    remaining = find_ida_pids()
    if remaining:
        print(f"\nERROR: {len(remaining)} IDA instance(s) still running. Will NOT force kill.")
        for pid, name in remaining.items():
            print(f"  [{pid}] {name}")
        return 1
    else:
