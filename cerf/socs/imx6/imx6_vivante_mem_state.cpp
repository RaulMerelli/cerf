...
void VivanteMem::StoreStateReg(uint32_t byte_off, uint32_t value) {
    ...
    if (byte_off == 0x0123Cu) { /* VIVS_DE_PATTERN_CONFIG */
        s_.state_[idx] = value;
        /* etnaviv rnndb/state_2d.xml: PATTERN_ADDRESS and
           PATTERN_CONFIG.INIT_TRIGGER load the DE pattern state. */
        ...
        if (pattern && init_trigger != 0u && pat_addr != 0u && bpp != 0u) {
            ...
            if (ReadGpuBytes(pat_addr, s_.de_pattern_latch_, bytes, MmuClient::Texture)) {
...
void VivanteMem::WriteMmuv2Configuration(uint32_t value) {
    ...
    if ((value & (1u << 3)) == 0u) cur = (cur & ~1u) | (value & 1u);
    if ((value & (1u << 7)) == 0u) cur &= ~(1u << 4);
    if ((value & (1u << 8)) == 0u) cur = (cur & ~0xFFFFFC00u) | (value & 0xFFFFFC00u);
    s_.regs_[kMmuv2Configuration >> 2] = cur;
    const uint32_t idx = kMmuv2Configuration >> 2;
    if (idx < s_.state_.size()) s_.state_[idx] = cur;
}
