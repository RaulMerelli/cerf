...
template <uint32_t kBase, int kSpi, bool kHasCard = true>
class Imx6UsdhcPort : public Peripheral {
public:
    ...
    void WriteWord(uint32_t a, uint32_t v) override {
        HandleWrite(a - kBase, v);
    }

    void SaveState(StateWriter& w) override {
        w.Write(cmdarg_);  w.Write(cmd_xfr_typ_); w.Write(mix_ctrl_); w.Write(irqstat_);
        w.Write(irqstaten_); w.Write(irqsigen_); w.Write(sys_ctrl_);
        w.Write(prot_ctrl_); w.Write(blk_att_); w.Write(wtmk_lvl_);
        w.Write(vend_spec_); w.Write(ds_addr_); w.Write(adma_sys_addr_);
        w.WriteBytes(rsp_, sizeof(rsp_));
        w.Write(buf_pos_); w.Write(blocks_rem_);
        const uint32_t flags = (buf_reading_ ? 1u : 0u)
                             | (buf_writing_ ? 2u : 0u)
                             | (next_is_acmd_ ? 4u : 0u)
                             | (open_ended_read_ ? 8u : 0u)
                             | (open_ended_write_ ? 16u : 0u);
        w.Write(flags);
        w.WriteBytes(buf_, sizeof(buf_));
    }

    void RestoreState(StateReader& r) override {
        r.Read(cmdarg_);  r.Read(cmd_xfr_typ_); r.Read(mix_ctrl_); r.Read(irqstat_);
        r.Read(irqstaten_); r.Read(irqsigen_); r.Read(sys_ctrl_);
        r.Read(prot_ctrl_); r.Read(blk_att_); r.Read(wtmk_lvl_);
        r.Read(vend_spec_); r.Read(ds_addr_); r.Read(adma_sys_addr_);
        r.ReadBytes(rsp_, sizeof(rsp_));
        r.Read(buf_pos_); r.Read(blocks_rem_);
        uint32_t flags = 0u; r.Read(flags);
...
private:
    uint32_t HandleRead(uint32_t off) {
        ...
        default:
            HaltUnsupportedAccess("imx6-usdhc read32 unmodelled register", kBase + off, 0);
        }
    }
    ...
    void HandleWrite(uint32_t off, uint32_t v) {
        ...
        case kCMD_XFR_TYP:
            cmd_xfr_typ_ = v;
            ExecuteCommand(cmd_xfr_typ_); return;
        ...
        case kSYS_CTRL:
            /* SWRST_ALL[24]/CMD[25]/DATA[26] and INITA[27] self-clear in HW
               (SDHCI spec Â§2.2.15 for resets; kINITA for 80-clock init sequence).
               CLOCK_INT_STABLE[1] is asserted immediately when CLOCK_INT_EN[0]
               is written â€” the driver polls bit 1 before touching any SD command. */
            ...
            if (sys_ctrl_ & kCLK_INT_EN)
                ...
                sys_ctrl_ &= ~kCLK_INT_STBL;
            return;
        case kIRQSTAT:    /* W1C â€” clear bits that the driver acknowledged. */
            irqstat_ &= ~v;
            ...
            if ((v & kErrorSpecificMask) != 0u &&
                (irqstat_ & kErrorSpecificMask) == 0u)
                irqstat_ &= ~kERRI;
            if ((v & kERRI) != 0u)
                irqstat_ &= ~kErrorSpecificMask;
            ...
            if ((v & kBRR) != 0u && buf_reading_ &&
                ...
                irqstat_ |= kBRR;
            if ((v & kBWR) != 0u && buf_writing_ &&
                (open_ended_write_ || buf_pos_ < std::max(4u, blk_att_ & 0x1FFFu)))
                irqstat_ |= kBWR;
            DeassertIfEmpty();
            return;
        case kIRQSTATEN:
            irqstaten_ = v;
            return;
        case kIRQSIGEN:   irqsigen_  = v;
            AssertIfEnabled(); return;
        ...
        default:
            HaltUnsupportedAccess("imx6-usdhc write32 unmodelled register", kBase + off, v);
        }
    }
    ...
    void ExecuteCommand(uint32_t cmd_xfr_typ) {
        ...
        const bool pure_busy_response = busy_response && !dpsel &&
                                        !res.starts_read && !res.starts_write;
        ...
        if (dpsel && dma_mode) {
            if (res.starts_read)       AdmaDmaRead();
    ...
    void AdmaDmaRead() {
        const uint32_t blksize = std::max(4u, blk_att_ & 0x1FFFu);
        const Imx6UsdhcAdma::Transfer transfer{
            adma_sys_addr_ ? adma_sys_addr_ : ds_addr_,
            blksize,
            (blk_att_ >> 16) & 0xFFFFu,
            (mix_ctrl_ & kMixBlkCntEn) != 0u
        };
    ...
    uint32_t PresentState() const {
        /* IMX6SDLRM Â§67.8.10: CINST[16]=0 and CDPL[18]=0 when no card present.
    ...
    void DeassertIfEmpty() {
        if (!(irqstat_ & irqsigen_))
            emu_.Get<::Imx6Gic>().DeAssertSpi(kSpi);
    }
...
/* imx6qdl.dtsi: uSDHC1-4 at 0x02190000/02194000/02198000/0219C000, GIC SPI 22/23/24/25. */
