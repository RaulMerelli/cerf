...
public:
    VivanteRs(VivanteState& s, VivanteMem& mem)
    ...
    void ExecuteRsInPlace(uint32_t tile_count) {
        ...
        if (tile_count != 0u) {
            LOG(Trace, "Imx6Gpu%s: ignoring unsupported RS in-place resolve "
                       "tiles=%u (not a normal RS copy)",
                mem_.CoreName(), tile_count);
        }
    }
    ...
    void ExecuteRs() {
        ...
        const bool src_supertiled =
            (src_stride_reg & (1u << 31)) != 0u || src_supertiled_new;
        const bool dst_supertiled =
            (dst_stride_reg & (1u << 31)) != 0u || dst_supertiled_new;
        ...
        if (dst_stride == 0u || dst_bpp == 0u || window_w == 0u ||
            window_h == 0u || (clear_mode == 0u &&
            (src_stride == 0u || src_bpp == 0u))) {
            return;
        }

        if ((src_multi || dst_multi) && mem_.PixelPipes() < 2u) {
            LOG(Trace, "Imx6Gpu%s: RS MULTI requested on a one-pipe core "
                       "src_stride=%08X dst_stride=%08X",
                mem_.CoreName(), src_stride_reg, dst_stride_reg);
            return;
        }

        uint32_t pipe_src_addr[2] = {
            mem_.StateReg(kRsPipeSourceAddr),
            mem_.StateReg(kRsPipeSourceAddr + 4u)
        };
        uint32_t pipe_dst_addr[2] = {
            mem_.StateReg(kRsPipeDestAddr),
            mem_.StateReg(kRsPipeDestAddr + 4u)
        };
        const uint32_t pipe_offset_reg[2] = {
            mem_.StateReg(kRsPipeOffset),
            mem_.StateReg(kRsPipeOffset + 4u)
        };
        const uint32_t pipe_offset_x[2] = {
            Lo16(pipe_offset_reg[0]), Lo16(pipe_offset_reg[1])
        };
        const uint32_t pipe_offset_y[2] = {
            Hi16(pipe_offset_reg[0]), Hi16(pipe_offset_reg[1])
        };

        uint32_t pipe_count = 1u;
        if (mem_.PixelPipes() >= 2u &&
            (src_multi || dst_multi || pipe_src_addr[1] != 0u ||
             pipe_dst_addr[1] != 0u || pipe_offset_reg[1] != 0u)) {
            pipe_count = 2u;
        }
        ...
        const uint32_t common_src_addr =
            (pipe_count > 1u && pipe_src_addr[0] != 0u)
                ? pipe_src_addr[0] : src_addr;
        const uint32_t common_dst_addr =
            (pipe_count > 1u && pipe_dst_addr[0] != 0u)
                ? pipe_dst_addr[0] : dst_addr;
        if (!src_multi) {
        ...
        for (uint32_t p = 0; p < pipe_count; ++p) {
            if (clear_mode == 0u) {
                if (pipe_src_addr[p] == 0u ||
                    !mem_.TranslateGpuToHost(pipe_src_addr[p]))
                    return;
            }
            if (pipe_dst_addr[p] == 0u ||
                !mem_.TranslateGpuToHostWrite(pipe_dst_addr[p]))
                return;
        }
        ...
        if (output_w == 0u || output_h == 0u)
            return;

        uint32_t source_full_height = 0u;
        for (uint32_t p = 0; p < pipe_count; ++p) {
            const uint32_t bottom = pipe_offset_y[p] + window_h;
            if (bottom > source_full_height)
                source_full_height = bottom;
        }

        const uint32_t ts_mem_config = mem_.StateReg(kTsMemConfig);
        const bool ts_color_fast_clear =
            mem_.SupportsFastClear() && (ts_mem_config & (1u << 1)) != 0u;
        const bool ts_color_compression =
            mem_.SupportsColorCompression() &&
            (ts_mem_config & (1u << 7)) != 0u;
        ...
        auto read_source_surface = [&](uint32_t base, uint32_t x, uint32_t y,
                                       uint32_t& argb) -> bool {
            const bool color_ts_targets_surface =
                (ts_color_fast_clear || ts_color_compression) &&
                ts_color_surface != 0u && base == ts_color_surface;
            if (color_ts_targets_surface) {
                if (ts_bits_per_tile == 0u || ts_color_status == 0u)
                    return false;
                const size_t byte_offset = SurfaceOffset(
                    src_stride, x, y, src_bpp, src_tiled, src_supertiled,
                    src_supertiled_new);
                ...
                if (!ReadPackedGpu(ts_color_status, bit_index >> 3, 1u, 0u,
                                   status_byte, MmuClient::Rasterizer)) {
                    return false;
                }
                const uint32_t bit_in_byte = static_cast<uint32_t>(bit_index & 7u);
                const uint32_t state_mask = (1u << ts_bits_per_tile) - 1u;
                const uint32_t tile_state =
                    (status_byte >> bit_in_byte) & state_mask;
            ...
            return ReadSurfacePixelGpu(base, src_stride, x, y, src_fmt, 0u,
                                       src_tiled, src_supertiled, argb,
                                       src_supertiled_new, 0u,
                                       MmuClient::Rasterizer);
        };

        auto read_source_global = [&](uint32_t gx, uint32_t gy,
                                      uint32_t& argb) -> bool {
            if (!src_multi) {
                return read_source_surface(pipe_src_addr[0], gx, gy, argb);
            }
            for (uint32_t p = 0; p < pipe_count; ++p) {
                if (gx < pipe_offset_x[p] || gy < pipe_offset_y[p] ||
                    gx >= pipe_offset_x[p] + window_w ||
                    gy >= pipe_offset_y[p] + window_h) {
                    continue;
                }
                return read_source_surface(
                    pipe_src_addr[p], gx - pipe_offset_x[p],
                    gy - pipe_offset_y[p], argb);
            }
            return false;
        };
        ...
        auto merge_clear_channels = [](uint32_t old_argb, uint32_t fill_argb,
                                       uint32_t nibble) -> uint32_t {
        ...
        for (uint32_t p = 0; p < pipe_count; ++p) {
            ...
            for (uint32_t dy = 0; dy < output_h; ++dy) {
                for (uint32_t dx = 0; dx < output_w; ++dx) {
                    ...
                    if (clear_mode != 0u) {
                        const uint32_t group = clear_mode == 1u
                            ? 0u : ((global_dx >> 2) & 3u);
                        const uint32_t nibble = (clear_bits >> (group * 4u)) & 0xFu;
                        if (nibble == 0u)
                            continue;
                        out_argb = UnpackSurfaceColor(
                            mem_.StateReg(kRsFillValue0 + group * 4u), src_fmt);
                        if (nibble != 0xFu) {
                            uint32_t old_argb = 0u;
                            if (!ReadSurfacePixelGpu(
                                    pipe_dst_addr[p], dst_stride, write_x, write_y,
                                    dst_fmt, 0u, dst_tiled, dst_supertiled,
                                    old_argb, dst_supertiled_new, 0u,
                                    MmuClient::PixelEngine)) {
                                continue;
                            }
                            out_argb = merge_clear_channels(old_argb, out_argb,
                                                            nibble);
                        }
                    } else {
                        ...
                        for (uint32_t sy = 0; sy < scale_y; ++sy) {
                            for (uint32_t sx = 0; sx < scale_x; ++sx) {
                                const uint32_t global_sx =
                                    pipe_offset_x[p] + dx * scale_x + sx;
                                uint32_t global_sy =
                                    pipe_offset_y[p] + dy * scale_y + sy;
                                if (flip_y)
                                    global_sy = source_full_height - 1u - global_sy;
                                uint32_t sample = 0u;
                                if (!read_source_global(global_sx, global_sy,
                                                        sample)) {
                                    continue;
                                }
                        ...
                        if (samples == 0u)
                            continue;
                        out_argb =
                            (((sums[0] + samples / 2u) / samples) << 24) |
                            (((sums[1] + samples / 2u) / samples) << 16) |
                            (((sums[2] + samples / 2u) / samples) << 8) |
                            ((sums[3] + samples / 2u) / samples);
                        if (swap_rb)
                            out_argb = swap_red_blue(out_argb);
                    }

                    WriteSurfacePixelGpu(pipe_dst_addr[p], dst_stride, write_x,
                                         write_y, dst_fmt, 0u, dst_tiled,
                                         dst_supertiled, out_argb,
                                         dst_supertiled_new);
                }
            }
        }
