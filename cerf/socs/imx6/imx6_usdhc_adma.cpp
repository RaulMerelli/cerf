...
void Imx6UsdhcAdma::Read(SdCard& card, const Transfer& transfer, uint8_t* block_buffer) {
    ...
    for (uint32_t offset = 0u; !transfer.count_limited || blocks_left > 0u; offset += 8u) {
        const uint8_t* descriptor = memory.TryTranslate(transfer.descriptor_base + offset);
        if (!descriptor) break;
        ...
        if (((attributes >> 4) & 0x03u) == 0x02u) {
            ...
            while (moved < length && (!transfer.count_limited || blocks_left > 0u)) {
                if (data_count == 0u) card.ReadBlock(block_buffer);
