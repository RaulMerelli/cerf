...
public:
    ...
    void PostRestore() override {
        /* The booleans are host-side mirrors of the GIC input levels, not
           independent hardware state. Re-drive them from IPU status/masks
           after the GIC and IPU register files have both been restored. */
...
private:
    void WriteMerged(uint32_t off, uint32_t value) {
        ...
        if (ipu_off == kOffMemRst) value &= ~kRstMemStart; /* internal-memory reset completes immediately */
    ...
    void EnsureModelledAccess(const char* what, uint32_t addr, uint32_t ipu_off, uint32_t value) const {
