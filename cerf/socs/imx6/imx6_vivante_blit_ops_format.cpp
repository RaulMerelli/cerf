...
uint32_t VivanteBlitOps::YuvToArgb(uint32_t y, uint32_t u, uint32_t v,
                                   bool bt709) {
    ...
    const int32_t r = bt709
        ? ((298 * a + 461 * c + 128) >> 8)
        : ((298 * a + 410 * c + 128) >> 8);
    const int32_t g = bt709
        ? ((298 * a - 55 * b - 137 * c + 128) >> 8)
        : ((298 * a - 101 * b - 209 * c + 128) >> 8);
    const int32_t blue = bt709
        ? ((298 * a + 543 * b + 128) >> 8)
        : ((298 * a + 519 * b + 128) >> 8);
...
VivanteBlitOps::ArgbChannels VivanteBlitOps::BlendFactor(
    uint32_t mode, const ArgbChannels& reference,
    ...
    case 2u: /* NORMAL */
        return {reference.a, reference.a, reference.a, reference.a};
    case 3u: /* INVERSED */
        return {255u - reference.a, 255u - reference.a,
                255u - reference.a, 255u - reference.a};
    case 4u: /* COLOR */
        return reference;
    case 5u: /* COLOR_INVERSED */
        return {255u - reference.a, 255u - reference.r,
                255u - reference.g, 255u - reference.b};
    case 6u: { /* SATURATED_ALPHA */
        const uint32_t sat = source_alpha < (255u - destination_alpha)
            ? source_alpha : (255u - destination_alpha);
        return {255u, sat, sat, sat};
    }
    default: { /* SATURATED_DEST_ALPHA */
        const uint32_t sat = destination_alpha < (255u - source_alpha)
...
uint32_t VivanteBlitOps::BlendPePixel(uint32_t src_argb, uint32_t dst_argb,
    ...
    const uint32_t global_src_alpha = pe20
        ? global_src.a : ((alpha_control >> 16) & 0xFFu);
    const uint32_t global_dst_alpha = pe20
        ? global_dst.a : ((alpha_control >> 24) & 0xFFu);
    src.a = EffectiveAlpha(src.a, global_src_alpha,
                           (alpha_modes & 1u) != 0u,
                           (alpha_modes >> 8) & 3u);
    dst.a = EffectiveAlpha(dst.a, global_dst_alpha,
                           (alpha_modes & (1u << 4)) != 0u,
                           (alpha_modes >> 12) & 3u);

    const bool src_premultiply = pe20
        ? ((color_multiply_modes & 1u) != 0u)
        : ((alpha_modes & (1u << 16)) != 0u);
    const bool dst_premultiply = pe20
        ? ((color_multiply_modes & (1u << 4)) != 0u)
        : ((alpha_modes & (1u << 20)) != 0u);
    if (src_premultiply) {
    ...
    const ArgbChannels src_factor = BlendFactor((alpha_modes >> 24) & 7u,
                                                 src_reference, src.a, dst.a);
    const ArgbChannels dst_factor = BlendFactor((alpha_modes >> 28) & 7u,
                                                 dst_reference, src.a, dst.a);

    auto add = [](uint32_t source, uint32_t sf,
                  uint32_t destination, uint32_t df) -> uint32_t {
        const uint32_t value = source * sf + destination * df;
        const uint32_t rounded = (value + 127u) / 255u;
        return rounded > 255u ? 255u : rounded;
    };
    ArgbChannels out{
        add(src.a, src_factor.a, dst.a, dst_factor.a),
        add(src.r, src_factor.r, dst.r, dst_factor.r),
        add(src.g, src_factor.g, dst.g, dst_factor.g),
        add(src.b, src_factor.b, dst.b, dst_factor.b)
    };
...
bool VivanteBlitOps::NativeColorKeyMatch(uint32_t packed, uint32_t low,
    ...
    for (uint32_t i = 0u; i < pixel.count; ++i) {
        if (pixel.value[i] < lo.value[i] || pixel.value[i] > hi.value[i])
            return false;
    }
    return true;
}

bool VivanteBlitOps::ReadVrYuvPixel(const uint8_t* y_plane, uint32_t y_stride,
                                    const uint8_t* u_plane, uint32_t u_stride,
                                    const uint8_t* v_plane, uint32_t v_stride,
                                    uint32_t x, uint32_t y, uint32_t fmt,
                                    uint32_t endian, bool uv_swizzle,
                                    bool bt709, uint32_t& argb) {
    if (!y_plane || y_stride == 0u)
        return false;
    ...
    case 7u: { /* YUY2: Y0 U Y1 V */
        const size_t pair = static_cast<size_t>(y) * y_stride +
    ...
    case 15u: { /* YV12 uses explicit U/V plane registers. */
        if (!u_plane || !v_plane || u_stride == 0u || v_stride == 0u)
            return false;
        yy = y_plane[EndianByteOffset(static_cast<size_t>(y) * y_stride + x,
                                      endian)];
        const size_t ux = x >> 1;
        const size_t uy = y >> 1;
...
void VivanteBlitOps::ApplyPe10ClearBytes(uint8_t* base, size_t offset,
                                         uint32_t pixel_bytes, uint32_t low,
                                         uint32_t high, uint32_t byte_mask,
                                         uint32_t endian) {
    if (!base || pixel_bytes == 0u)
        return;
    const uint64_t pattern = static_cast<uint64_t>(low) |
    ...
    for (uint32_t byte = 0u; byte < pixel_bytes; ++byte) {
        ...
        if ((byte_mask & (1u << slot)) == 0u)
            continue;
        base[EndianByteOffset(offset + byte, endian)] =
            static_cast<uint8_t>(pattern >> (slot * 8u));
    }
}

uint32_t VivanteBlitOps::UnpackSurfaceColor(uint32_t packed, uint32_t fmt,
                                            uint32_t swizzle) {
    switch (fmt & 0x1Fu) {
    case 0:
        return Rgba4444ToArgb(static_cast<uint16_t>(packed), false);
    case 1:
        return Rgba4444ToArgb(static_cast<uint16_t>(packed), true);
    case 2:
        return Rgb555ToArgb(static_cast<uint16_t>(packed), false);
    case 3:
        return Rgb555ToArgb(static_cast<uint16_t>(packed), true);
    case 4:
        return Rgb565ToArgb(static_cast<uint16_t>(packed));
    case 5:
        return ApplyReadSwizzle(packed | 0xFF000000u, swizzle);
    case 6:
        return ApplyReadSwizzle(packed, swizzle);
    case 9:
        return 0xFF000000u | ((packed & 0xFFu) * 0x010101u);
    case 16:
        return ((packed & 0xFFu) << 24) | 0x00FFFFFFu;
    default:
        return NormalizeArgb(packed);
    }
}

uint32_t VivanteBlitOps::PackSurfaceColor(uint32_t argb, uint32_t fmt,
                                          uint32_t swizzle) {
    switch (fmt & 0x1Fu) {
    case 0:
        return ArgbToRgba4444(argb, false);
    case 1:
        return ArgbToRgba4444(argb, true);
    case 2:
        return ArgbToRgb555(argb, false);
    case 3:
        return ArgbToRgb555(argb, true);
    case 4:
        return ArgbToRgb565(argb);
    case 5:
        return ApplyWriteSwizzle(argb | 0xFF000000u, swizzle);
    case 6:
        return ApplyWriteSwizzle(argb, swizzle);
    case 9:
    case 16:
