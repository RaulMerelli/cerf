...
bool VivanteBlitOps::ResolveSourceCoordinate(uint32_t origin_field, bool relative, uint32_t destination_coordinate,
...
bool VivanteBlitOps::ResolveMonoStreamCoordinate(uint32_t origin_field, bool relative, uint32_t destination_coordinate,
...
bool VivanteBlitOps::ResolveMonoStreamExtent(uint32_t origin_field, bool relative, uint32_t rectangle_extent,
...
uint32_t VivanteBlitOps::StretchCoordinate(uint32_t destination_offset, uint32_t source_extent,
...
uint32_t VivanteBlitOps::SurfaceWidthFromStride(uint32_t stride, uint32_t bpp, SurfaceLayout layout) {
...
VivanteBlitOps::SurfaceLayout VivanteBlitOps::DecodeSurfaceLayout(bool tiled, bool multi_tiled, bool supertiled,
                                                                  bool minor_tiled) {
...
VivanteBlitOps::SurfaceLocation VivanteBlitOps::LocateSurface(uint32_t stride, uint32_t x, uint32_t y, uint32_t bpp,
...
VivanteBlitOps::DeCoord VivanteBlitOps::TransformDeCoord(uint32_t x, uint32_t y, uint32_t width, uint32_t height,
    ...
    uint32_t ty = y;
    switch (rot) {
    case 1: /* FLIP_X in DE_ROT_MODE */ tx = (width > 1u && x < width) ? (width - 1u - x) : x; break;
    case 2: /* FLIP_Y */ ty = (height > 1u && y < height) ? (height - 1u - y) : y; break;
    case 4: /* ROT90 */
