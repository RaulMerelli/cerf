...
struct VivanteBlitOps {
    ...
    static bool ResolveSourceCoordinate(uint32_t origin_field, bool relative, uint32_t destination_coordinate,
    ...
    static bool ResolveRopResource(bool dependency, uint32_t override_mode) {
        return override_mode == 1u ? true : override_mode == 2u ? false : dependency;
    }

    static uint32_t SelectSourceTransparency(bool pe20, uint32_t pe_transparency, uint32_t legacy_transparency) {
        return pe20 ? (pe_transparency & 3u) : (legacy_transparency & 3u);
    }

    static bool ResolveMonoStreamCoordinate(uint32_t origin_field, bool relative, uint32_t destination_coordinate,
                                            uint32_t rectangle_offset, uint32_t& stream_coordinate);
    static bool ResolveMonoStreamExtent(uint32_t origin_field, bool relative, uint32_t rectangle_extent,
                                        uint32_t& stream_extent);
    static uint32_t StretchCoordinate(uint32_t destination_offset, uint32_t source_extent, uint32_t destination_extent,
                                      uint32_t programmed_factor, bool gdi_stretch);
    ...
    static uint32_t Rgba4444ToArgb(uint16_t px, bool alpha) {
        ...
        return ((a4 * 0x11u) << 24) | ((r4 * 0x11u) << 16) | ((g4 * 0x11u) << 8) | (b4 * 0x11u);
    }
    ...
    static uint32_t NormalizeArgb(uint32_t argb) { return (argb & 0xFF000000u) ? argb : (argb | 0xFF000000u); }
    ...
    static ArgbChannels SplitArgb(uint32_t argb) {
        return {(argb >> 24) & 0xFFu, (argb >> 16) & 0xFFu, (argb >> 8) & 0xFFu, argb & 0xFFu};
    }
    ...
    static uint32_t BlendPePixel(uint32_t src_argb, uint32_t dst_argb, uint32_t alpha_control, uint32_t alpha_modes,
                                 uint32_t color_multiply_modes, uint32_t global_src_color, uint32_t global_dst_color,
                                 bool pe20);
    ...
    static uint32_t UnpackSurfaceColor(uint32_t packed, uint32_t fmt, uint32_t swizzle = 0u);
    static uint32_t PackSurfaceColor(uint32_t argb, uint32_t fmt, uint32_t swizzle = 0u);
    ...
    static uint32_t SurfaceWidthFromStride(uint32_t stride, uint32_t bpp, SurfaceLayout layout);
    static SurfaceLayout DecodeSurfaceLayout(bool tiled, bool multi_tiled, bool supertiled, bool minor_tiled);
    static SurfaceLocation LocateSurface(uint32_t stride, uint32_t x, uint32_t y, uint32_t bpp, SurfaceLayout layout,
                                         bool supertiled_new = false);

    static size_t SurfaceOffset(uint32_t stride, uint32_t x, uint32_t y, uint32_t bpp, bool tiled, bool supertiled,
                                bool supertiled_new = false) {
        const SurfaceLayout layout =
            supertiled ? SurfaceLayout::SuperTiled : (tiled ? SurfaceLayout::Tiled : SurfaceLayout::Linear);
        return LocateSurface(stride, x, y, bpp, layout, supertiled_new).offset;
    }

    template <typename PixelFn>
    static uint32_t RasterizeLineBresenhamExclusive(uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1,
                                                    PixelFn&& emit_pixel) {
    ...
    static uint32_t RopCode(uint32_t rop, bool foreground = true) {
        ...
        if (type == 3u) return foreground ? (rop & 0xFFu) : ((rop >> 8) & 0xFFu);
        return rop & 0xFFu;
    }

    static bool RopTruthTableDependsOn(uint32_t code, uint32_t type, uint32_t input_bit);
    ...
    static MonoRopSelection SelectMonoRop(bool mono_one, bool mono_transparency_enabled, bool mono_transparent_one);

    static bool AddressRangesOverlap(uintptr_t a_begin, uintptr_t a_end, uintptr_t b_begin, uintptr_t b_end) {
    ...
    static bool RopCopyLike(uint32_t rop) {
        ...
        return (type == 1u && (fg & 0xFu) == 0xCu) || (type >= 2u && fg == 0xCCu);
    }

    static uint32_t ApplyRop(uint32_t rop, uint32_t dst, uint32_t src, uint32_t pat, bool foreground = true);
    static uint32_t ReadStreamMonoBit(const uint8_t* stream, uint32_t x, uint32_t y, uint32_t width, uint32_t pack,
                                      uint32_t stream_words);

    static bool ReadPatternBit(uint32_t low, uint32_t high, uint32_t x, uint32_t y) {
    ...
    static void WritePixel(uint8_t* dst, uint32_t bpp, uint32_t color) { std::memcpy(dst, &color, bpp); }
