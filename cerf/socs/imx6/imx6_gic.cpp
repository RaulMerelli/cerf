...
public:
    ...
    void AssertSpi(int spi) override {
        ...
        bool deliver = false;
        {
            ...
            if (w < 5) {
                ...
                line_level_[w] |= bit;
                if (!IsEdgeTriggeredLocked(gic_id) || !was_high)
                    pending_[w] |= bit;
            }
    ...
    void DeAssertSpi(int spi) override {
        ...
        if (w < 5) {
            line_level_[w] &= ~bit;
            if (!IsEdgeTriggeredLocked(gic_id))
                pending_[w] &= ~bit;
        }
    }
    ...
    uint32_t ReadMmio(uint32_t off) override {
        ...
        case 0x10C: {                      /* GICC_IAR */
            ...
            if (active_irq_ == 1023u && (distributor_control_ & 1u) &&
                (cpu_control_ & 1u)) {
        ...
        LOG(Caution, "[GIC] unsupported read32 at 0x%08X\n", kMmioBase + off);
        CerfFatalExit();
    }
    ...
    void WriteMmio(uint32_t off, uint32_t value) override {
        ...
        case 0x110: {                     /* GICC_EOIR */
            bool deliver = false;
            {
                ...
                deliver = (active_irq_ == 1023u) &&
                          (PrivateTimerIrqPendingLocked() || AnySpiPendingLocked());
            }
            if (deliver) emu_.Get<ArmJit>().SetInterruptPending();
            return;
        }
        ...
        case 0x608: {
            ...
            if ((value & 1u) != 0 && private_timer_counter_ == 0)
        ...
        if (off >= 0x1080u && off < 0x1094u) { groups_   [(off - 0x1080u) >> 2]  = value; return; }
        ...
        if (off >= 0x1200u && off < 0x1214u) { pending_  [(off - 0x1200u) >> 2] |= value; return; }
        ...
        if (off >= 0x1400u && off < 0x14A0u) { priorities_[(off - 0x1400u) >> 2]  = value; return; }
        if (off >= 0x1800u && off < 0x18A0u) { targets_  [(off - 0x1800u) >> 2]   = value; return; }
        ...
        LOG(Caution, "[GIC] unsupported write32 at 0x%08X val=0x%08X\n", kMmioBase + off, value);
        CerfFatalExit();
    }

    void SaveGicState(StateWriter& w) override {
        std::lock_guard<std::mutex> lk(timer_mutex_);
        w.Write(pt_anchor_cycles_); w.Write(active_irq_);
        w.Write(cpu_control_);      w.Write(priority_mask_);
        w.Write(binary_point_);
        w.Write(private_timer_load_);    w.Write(private_timer_counter_);
        w.Write(private_timer_control_); w.Write(private_timer_status_);
    ...
    void RestoreGicState(StateReader& r) override {
        std::lock_guard<std::mutex> lk(timer_mutex_);
        r.Read(pt_anchor_cycles_); r.Read(active_irq_);
        r.Read(cpu_control_);      r.Read(priority_mask_);
        r.Read(binary_point_);
        r.Read(private_timer_load_);     r.Read(private_timer_counter_);
        r.Read(private_timer_control_);  r.Read(private_timer_status_);
...
private:
    ...
    void AdvancePrivateTimerLocked(uint32_t cycles_now) {
        ...
        const uint64_t period = static_cast<uint64_t>(start + 1u) *
                    presc * kPeriphDiv;
        if (period == 0) return;
        ...
        if (elapsed >= period) {
            ...
            if ((private_timer_control_ & 2u) != 0) {
                const uint64_t reload_period =
                    static_cast<uint64_t>(private_timer_load_ + 1u) *
                    presc * kPeriphDiv;
    ...
    uint32_t ComputePrivateCounterLocked(uint32_t cycles_now) const {
        ...
        const uint32_t pos = static_cast<uint32_t>(
            (cycles_now - pt_anchor_cycles_) / cyc_per_tick);
    ...
    uint32_t ComputePrivateCounterFast(uint32_t cycles_now) const {
        ...
        const uint32_t pos = static_cast<uint32_t>(
            (cycles_now - pt_anchor_cycles_fast_.load(std::memory_order_relaxed)) / cyc_per_tick);
        return (pos > start) ? 0u : (start - pos);
    }
    ...
    void RefreshLevelPendingLocked() {
        for (int gic_id = 32; gic_id < 160; ++gic_id) {
            const int w = gic_id >> 5;
            const uint32_t bit = 1u << (gic_id & 31);
            if ((line_level_[w] & bit) != 0u && !IsEdgeTriggeredLocked(gic_id))
                pending_[w] |= bit;
        }
    }
