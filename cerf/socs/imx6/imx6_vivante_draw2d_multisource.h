...
protected:
    ...
    bool ReadMultiSource(const MultiSourceDesc& m, uint32_t x, uint32_t y, uint32_t& argb, uint32_t& packed) {
        if (!m.valid || m.stride == 0u || m.bpp == 0u) return false;
        DeCoord p{x, y};
        if ((ValidDeRot(m.rotation) != 0u || m.mirror != 0u) && m.surface_w != 0u && m.surface_h != 0u) {
            p = TransformDeCoord(x, y, m.surface_w, m.surface_h, m.rotation, m.mirror);
        }
        if (!ReadSurfacePackedGpuLayout(m.address, m.extra_address, m.stride, p.x, p.y, m.format, m.layout, packed,
                                        false, m.endian))
            return false;
        if (m.format == 16u) {
    ...
    bool ExecuteMultiSourceRect(uint32_t raw_x0, uint32_t raw_y0, uint32_t x0, uint32_t y0, uint32_t w, uint32_t h) {
        ...
        for (uint32_t y = 0u; y < h; ++y) {
            for (uint32_t x = 0u; x < w; ++x) {
                ...
                if (!ReadDstActual(x0 + x, y0 + y, current, dst_packed)) continue;
                ...
                for (uint32_t i = 0u; i < source_count; ++i) {
                    ...
                    if (!ReadMultiSource(m, sx, sy, src_argb, src_packed)) continue;
