...
private:
    ...
    void WriteMerged(uint32_t address, uint32_t value, uint32_t width) {
        ...
        Gic().WriteMmio(aligned - MmioBase(),
                        (old_value & ~(lane_mask << shift)) |
                        ((value & lane_mask) << shift));
    }
};
