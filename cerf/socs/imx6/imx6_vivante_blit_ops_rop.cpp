...
bool VivanteBlitOps::RopTruthTableDependsOn(uint32_t code, uint32_t type, uint32_t input_bit) {
...
VivanteBlitOps::MonoRopSelection VivanteBlitOps::SelectMonoRop(bool mono_one, bool mono_transparency_enabled,
...
uint32_t VivanteBlitOps::ApplyRop(uint32_t rop, uint32_t dst, uint32_t src, uint32_t pat, bool foreground) {
    const uint32_t type = (rop >> 20) & 3u;
    const uint32_t code = RopCode(rop, foreground);
...
uint32_t VivanteBlitOps::ReadStreamMonoBit(const uint8_t* stream, uint32_t x, uint32_t y, uint32_t width, uint32_t pack,
