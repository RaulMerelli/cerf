...
protected:
    ...
    void LoadState() {
        ...
        dst_layout_ =
            dst_minor_tiled_ ? SurfaceLayout::MinorTiled : (dst_tiled_ ? SurfaceLayout::Tiled : SurfaceLayout::Linear);
        ...
        src_fmt_ = pe20_ ? ((src_cfg_ >> 24) & 0x1Fu) : (src_cfg_ & 0xFu);
        ...
        src_layout_conflict_ = src_minor_tiled_ && (src_multi_tiled_ || src_supertiled_);
        src_layout_ = DecodeSurfaceLayout(src_tiled_, src_multi_tiled_, src_supertiled_, src_minor_tiled_);
        ...
        if (src_stream_) {
            ...
            const uint32_t block_fmt = pe20_ ? ((block_cfg >> 24) & 0x1Fu) : (block_cfg & 0xFu);
            const uint32_t block_pack = (block_cfg >> 12) & 3u;
            const uint32_t block_size = mem_.StateReg(kD2dMultiSrcSize);
            if (block_stream && block_fmt == 10u && block_pack == src_pack_ && block_size != 0u) {
        ...
        clear_argb_ =
            pe20_ ? mem_.StateReg(kD2dClearPe20) : UnpackSurfaceColor(mem_.StateReg(kD2dClearPe10Lo), dst_fmt_);
        ...
        if ((pe_transparency & 0x20000000u) != 0u) {
            /* etnaviv rnndb/state_2d.xml: PE_TRANSPARENCY.DFB_COLOR_KEY bit 29. */
            mem_.HaltUnsupported("PE_TRANSPARENCY.DFB_COLOR_KEY", kD2dPeTransparency, pe_transparency);
        }
        ...
        effective_src_transparency_ = SelectSourceTransparency(pe20_, pe_src_transparency_, src_transparency_);
        ...
        use_source_ = alpha_enable_ ? true : ResolveRopResource(rop_uses_source_, use_src_override_);
        use_pattern_ = alpha_enable_ ? false : ResolveRopResource(rop_uses_pattern_, use_pat_override_);
        use_destination_ = (alpha_enable_ || pe_dst_transparency_ == 2u)
                               ? true
        ...
        src_rot_ = ((rot_angle_ >> 8) & 1u) ? (rot_angle_ & 7u) : (((src_rot_cfg_ >> 16) & 1u) ? 4u : 0u);
        dst_rot_ = ((rot_angle_ >> 9) & 1u) ? ((rot_angle_ >> 3) & 7u) : (((dst_rot_cfg_ >> 16) & 1u) ? 4u : 0u);
        src_mirror_ = ((rot_angle_ >> 15) & 1u) ? ((rot_angle_ >> 12) & 3u) : 0u;
        dst_mirror_ = ((rot_angle_ >> 19) & 1u) ? ((rot_angle_ >> 16) & 3u) : 0u;
        src_surface_w_ = (src_rot_cfg_ & 0xFFFFu)
                             ? (src_rot_cfg_ & 0xFFFFu)
                             : ((src_stride_ && src_bpp_) ? SurfaceWidthFromStride(src_stride_, src_bpp_, src_layout_)
                                                          : (src_size_x_ ? src_size_x_ : 0u));
        src_surface_h_ = (mem_.StateReg(kD2dSrcRotHeight) & 0xFFFFu) ? (mem_.StateReg(kD2dSrcRotHeight) & 0xFFFFu)
                                                                     : (src_size_y_ ? src_size_y_ : 0u);
        dst_surface_w_ =
            (dst_rot_cfg_ & 0xFFFFu)
                ? (dst_rot_cfg_ & 0xFFFFu)
                : ((dst_stride_ && dst_bpp_) ? SurfaceWidthFromStride(dst_stride_, dst_bpp_, dst_layout_) : 0u);
    ...
    DeCoord DstCoord(uint32_t x, uint32_t y) const {
        if ((ValidDeRot(dst_rot_) == 0u && dst_mirror_ == 0u) || dst_surface_w_ == 0u || dst_surface_h_ == 0u)
            return {x, y};

        /* hmi_ktp400_mobile_v13 ddraw_ipu.dll sub_EF540980 reverses mono-blit bands
        ...
        if ((dst_rot_ & 7u) == 0u && dst_mirror_rect_valid_) {
            DeCoord p{x, y};
            if ((dst_mirror_ & 1u) != 0u && p.x >= dst_mirror_rect_x0_ && p.x < dst_mirror_rect_x1_) {
                p.x = dst_mirror_rect_x0_ + (dst_mirror_rect_x1_ - 1u - p.x);
            }
            if ((dst_mirror_ & 2u) != 0u && p.y >= dst_mirror_rect_y0_ && p.y < dst_mirror_rect_y1_) {
                p.y = dst_mirror_rect_y0_ + (dst_mirror_rect_y1_ - 1u - p.y);
            }
            return p;
        }
        return TransformDeCoord(x, y, dst_surface_w_, dst_surface_h_, dst_rot_, dst_mirror_);
    }

    bool ReadDstActual(uint32_t x, uint32_t y, uint32_t& argb, uint32_t& packed) {
        const DeCoord p = DstCoord(x, y);
        if (!ReadSurfacePackedGpuLayout(dst_addr_, 0u, dst_stride_, p.x, p.y, dst_fmt_, dst_layout_, packed, false,
                                        dst_endian_, MmuClient::PixelEngine))
            return false;
    ...
    bool WriteDst(uint32_t x, uint32_t y, uint32_t argb) {
        const DeCoord p = DstCoord(x, y);
        return WriteSurfacePixelGpuLayout(dst_addr_, 0u, dst_stride_, p.x, p.y, dst_fmt_, dst_swizzle_, dst_layout_,
                                          argb, false, dst_endian_);
    }

    bool FastSolidPatternFill(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h) {
        if (dst_layout_ != SurfaceLayout::Linear || dst_bpp_ != 4u || dst_endian_ != 0u || dst_cmd_ != 2u ||
            alpha_enable_ || use_destination_ || pe_dst_transparency_ == 2u || pe_pat_transparency_ == 1u || pat_ptr_ ||
            (pat_cfg_ & (1u << 4)) || ValidDeRot(dst_rot_) != 0u || (dst_mirror_ & 1u) != 0u)
            return false;

        const uint32_t pat_argb = PatternPixel(x0, y0, clear_argb_);
        const uint32_t out_argb = ApplyRop(rop_, 0u, pat_argb, pat_argb);
        const uint32_t packed = PackSurfaceColor(out_argb, dst_fmt_, dst_swizzle_);
        std::vector<uint32_t> row(w, packed);
        for (uint32_t y = 0u; y < h; ++y) {
            const DeCoord p = DstCoord(x0, y0 + y);
            const size_t offset = static_cast<size_t>(p.y) * dst_stride_ + static_cast<size_t>(p.x) * dst_bpp_;
            uint32_t address = 0u;
            if (!AddGpuOffset(dst_addr_, offset, address) ||
                !mem_.WriteGpuBytes(address, row.data(), static_cast<size_t>(w) * dst_bpp_, MmuClient::PixelEngine))
                return false;
        }
        return true;
    }
    ...
    void ClearDst(uint32_t x, uint32_t y) {
        ...
        const DeCoord p = DstCoord(x, y);
        const size_t offset = LocateSurface(dst_stride_, p.x, p.y, dst_bpp_, dst_layout_).offset;
        ApplyPe10ClearGpu(dst_addr_, offset, dst_bpp_, mem_.StateReg(kD2dClearPe10Lo), mem_.StateReg(kD2dClearPe10Hi),
                          mem_.StateReg(kD2dClearByteMask) & 0xFFu, dst_endian_);
    }

    bool SourceTransparent(uint32_t packed) const {
        return effective_src_transparency_ == 2u ? NativeColorKeyMatch(packed, src_key_low_, src_key_high_, src_fmt_)
                                                 : false;
    }

    bool DestinationAccepts(uint32_t packed) const {
        if (pe_dst_transparency_ != 2u) return true;
        return NativeColorKeyMatch(packed, dst_key_low_, dst_key_high_, dst_fmt_);
    }

    bool PatternOpaque(uint32_t x, uint32_t y) const {
        if (pe_pat_transparency_ != 1u) return true;
        if ((pat_mask_low_ | pat_mask_high_) == 0u) return true;
        const uint32_t origin_x = (pat_cfg_ >> 16) & 7u;
        const uint32_t origin_y = (pat_cfg_ >> 20) & 7u;
        return ReadPatternBit(pat_mask_low_, pat_mask_high_, x + origin_x, y + origin_y);
    }

    bool PatternAccepts(uint32_t x, uint32_t y) const { return PatternOpaque(x, y); }

    bool ReadPatternMemory(uint32_t x, uint32_t y, uint32_t& argb) {
        if ((!pat_ptr_ && !s_.de_pattern_latch_valid_) || pat_bpp_ == 0u) return false;
        ...
        const bool use_latch = s_.de_pattern_latch_valid_ && s_.de_pattern_latch_config_ == pat_cfg_ &&
                               s_.de_pattern_latch_address_ == pat_addr_ && s_.de_pattern_latch_bpp_ == pat_bpp_;
        const size_t latch_offset = (static_cast<size_t>(py) * 8u + px) * pat_bpp_;
    ...
    uint32_t PatternPixel(uint32_t x, uint32_t y, uint32_t fallback) {
        uint32_t argb = 0u;
        if (ReadPatternMemory(x, y, argb)) return argb;
        return VivanteBlitOps::PatternPixel(x, y, pat_cfg_, pat_low_, pat_high_, pat_bg_, pat_fg_, fallback);
    }

    DeCoord SrcCoord(uint32_t x, uint32_t y) const {
        if ((ValidDeRot(src_rot_) == 0u && src_mirror_ == 0u) || src_surface_w_ == 0u || src_surface_h_ == 0u)
            return {x, y};
        return TransformDeCoord(x, y, src_surface_w_, src_surface_h_, src_rot_, src_mirror_);
    }

    bool ReadSource(uint32_t x, uint32_t y, uint32_t& argb, uint32_t& packed) {
        const DeCoord sp = SrcCoord(x, y);
        if (!src_surface_configured_) return false;
        if (!ReadSurfacePackedGpuLayout(src_addr_, src_ex_addr_, src_stride_, sp.x, sp.y, src_fmt_, src_layout_, packed,
                                        false, src_endian_))
            return false;
