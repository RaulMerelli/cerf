...
public:
    ...
    uint32_t RequestTransmit(EmulatedMemory& memory, NetworkBackend& network, bool controller_enabled,
                             bool cable_connected, uint32_t tx_base);
    uint32_t Receive(EmulatedMemory& memory, const uint8_t* frame, std::size_t length, uint32_t rx_base,
