...
class VivanteFe {
public:
    VivanteFe(VivanteState& s, VivanteMem& mem, VivanteBlit& blit)
        : s_(s), mem_(mem), blit_(blit) {}

    void AdvanceFrontendRing() {
        if (!s_.fe_live_ || s_.fe_ring_pc_ == 0u || s_.fe_in_advance_)
            return;
        s_.fe_in_advance_ = true;

        if (s_.fe_idle_ring_) {
            uint32_t w0 = 0u;
            mem_.ReadCommandWords(s_.fe_ring_pc_, &w0, 1u,
                                  s_.fe_address_space_);
        ...
        const uint32_t final_pc = ExecuteCommandStream(
            start_pc, s_.fe_ring_prefetch_, stats, true,
            s_.fe_address_space_);
    ...
    void RunFrontend(uint32_t control) {
        ...
        constexpr FeCommandAddressSpace kBootstrapSpace =
            FeCommandAddressSpace::Physical;
        mem_.DumpCommandWords(address, prefetch, kBootstrapSpace);

        IdleRingInfo idle;
        if (prefetch >= 2u &&
            mem_.DetectIdleRing(address, kBootstrapSpace, idle)) {
        ...
        FeStats stats;
        const uint32_t final_pc = ExecuteCommandStream(
            address, prefetch, stats, false, kBootstrapSpace);
        s_.regs_[0x664u >> 2] = final_pc;
    ...
    bool CommandFits(uint32_t command_words, uint32_t window_words,
                     uint32_t pc, FeStats& stats) {
        if (command_words <= window_words)
            return true;
        stats.blocked = true;
    ...
    bool FetchWords(uint32_t pc, uint32_t* words, uint32_t count,
                    FeCommandAddressSpace address_space,
                    uint32_t window_words, FeStats& stats) {
        if (mem_.ReadCommandWords(pc, words, count, address_space))
            return true;
        stats.blocked = true;
        s_.fe_window_words_ = window_words;
        s_.regs_[0x664u >> 2] = pc;
    ...
    void ConsumeWords(uint32_t words, uint32_t& pc,
                      uint32_t& window_words) const {
        pc += words * 4u;
        window_words -= words;
    }

    uint32_t ExecuteCommandStream(uint32_t address, uint32_t prefetch,
                                  FeStats& stats, bool resume,
                                  FeCommandAddressSpace start_space) {
        uint32_t pc = address;
        uint32_t window_words = resume
            ? s_.fe_window_words_ : FePrefetchDwords(prefetch);
        FeCommandAddressSpace address_space = resume
            ? s_.fe_address_space_ : start_space;
        if (!resume) {
            s_.fe_call_depth_ = 0u;
            s_.fe_resume_idle_target_ = 0u;
        }
        ...
        for (uint32_t step = 0; step < 512u; ++step) {
            ...
            if (!CommandFits(2u, window_words, pc, stats))
                return finish(pc);

            s_.regs_[0x664u >> 2] = pc;

            uint32_t words[4]{};
            if (!FetchWords(pc, words, 2u, address_space,
                            window_words, stats)) {
                return finish(pc);
            }
            ...
            case kFeLoadState: {
                ...
                if (!CommandFits(len, window_words, pc, stats))
                    return finish(pc);
                std::vector<uint32_t> packet(len);
                if (!FetchWords(pc, packet.data(), len, address_space,
                                window_words, stats)) {
                    return finish(pc);
                }
                ApplyLoadState(packet.data() + 1u, state_word, count,
                               fixp, stats);
                ++stats.load_state;
            ...
            case kFeSnapPages: {
                ...
                if (op == kFeWait)
                    ++stats.waits;
                if (op == kFeChipSelect)
                    s_.chip_select_mask_ = cmd & 0xFFFFu;
                ConsumeWords(len, pc, window_words);
                break;
            }
            ...
            case kFeDraw2d: {
                if (!mem_.Is2d())
                    mem_.HaltUnsupported(
                        "imx6-vivante DRAW_2D on non-GC320 core", pc, cmd);
                const uint32_t rects = ((cmd >> 8) & 0xFFu)
                    ? ((cmd >> 8) & 0xFFu) : 256u;
                const uint32_t data_count = (cmd >> 16) & 0x7FFu;
                const uint32_t len =
                    FeDraw2dPacketWords(rects, data_count);
                if (!CommandFits(len, window_words, pc, stats))
                    return finish(pc);
                ...
                ++stats.draw_2d;
                blit_.ExecuteDraw2d(packet.data() + 2u, rects,
                    packet.data() + 2u + rects * 2u, data_count);
                ConsumeWords(len, pc, window_words);
                break;
            }
            ...
            case kFeWaitFence: {
                ...
                if (!mem_.ReadMemoryU64(fence_address, actual))
                    mem_.HaltUnsupported(
                        "imx6-vivante WAIT_FENCE unreadable address",
                        fence_address, cmd);
                const uint64_t expected =
                    static_cast<uint64_t>(mem_.StateReg(0x007E8u)) |
            ...
            case kFeCall: {
                ...
                if (!CommandFits(len, window_words, pc, stats))
                    return finish(pc);
                if (!FetchWords(pc, words, len, address_space,
                                window_words, stats)) {
                    return finish(pc);
                }
                if (s_.fe_call_depth_ >= kFeCallStackDepth)
                    mem_.HaltUnsupported(
                        "imx6-vivante FE CALL stack overflow", pc, cmd);
            ...
            case kFeReturn:
                if (s_.fe_call_depth_ == 0u)
                    mem_.HaltUnsupported(
                        "imx6-vivante FE RETURN stack underflow", pc, cmd);
                {
                    const FeCallFrame frame =
                        s_.fe_call_stack_[--s_.fe_call_depth_];
            ...
            default:
                mem_.HaltUnsupported("imx6-vivante FE unknown opcode", pc, cmd);
            }
        }
    ...
    void ApplyLoadState(const uint32_t* values, uint32_t state_word,
                        uint32_t count, bool fixp, FeStats& stats) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            if (fixp) {
                const float converted =
                    static_cast<float>(static_cast<int32_t>(v)) / 65536.0f;
        ...
        if (rs_inplace_kick)
            blit_.ExecuteRsInPlace(rs_inplace_tiles);
