...
public:
    VivanteVr(VivanteState& s, VivanteMem& mem)
    ...
    void Execute(uint32_t start_value) {
        ...
        if (mode > 2u) {
            LOG(Trace, "Imx6Gpu%s: VR unsupported START=%u", mem_.CoreName(), mode);
            return;
        }
        ...
        const bool src_layout_conflict = src_minor_tiled &&
            (src_multi_tiled || src_supertiled);
        const SurfaceLayout src_layout = DecodeSurfaceLayout(
            src_tiled, src_multi_tiled, src_supertiled, src_minor_tiled);
        ...
        const bool src_format_supported = src_fmt <= 6u || src_fmt == 9u ||
                                          src_fmt == 16u || src_yuv;
        const uint32_t src_bpp = src_yuv ? 0u :
            (src_format_supported ? BppFromDeFormat(src_fmt) : 0u);
        ...
        const SurfaceLayout dst_layout = dst_minor_tiled
            ? SurfaceLayout::MinorTiled
            : (dst_tiled ? SurfaceLayout::Tiled : SurfaceLayout::Linear);
        const bool dst_format_supported = dst_fmt <= 6u || dst_fmt == 16u;
        const uint32_t dst_bpp = dst_format_supported
            ? BppFromDeFormat(dst_fmt) : 0u;

        if (!src_format_supported || !dst_format_supported) {
            LOG(Trace, "Imx6Gpu%s: VR unsupported format src=%u dst=%u",
                mem_.CoreName(), src_fmt, dst_fmt);
            return;
        }

        if (src_yuv && !yuv_to_rgb) {
            LOG(Trace, "Imx6Gpu%s: VR YUV source %s without PE_CONTROL.YUVRGB",
                mem_.CoreName(), DeFormatName(src_fmt));
            return;
        }
        ...
        if (src_layout_conflict || (planar_yuv && IsMultiLayout(src_layout))) {
            LOG(Trace, "Imx6Gpu%s: VR conflicting/unsupported YUV layout %s "
                       "src_ex=%08X fmt=%s",
                mem_.CoreName(), SurfaceLayoutName(src_layout), src_ex_cfg,
                DeFormatName(src_fmt));
            return;
        }

        const bool src_base_valid =
            src_addr && src_stride && (src_bpp || src_yuv) &&
            mem_.TranslateGpuToHost(src_addr) != nullptr &&
            (!IsMultiLayout(src_layout) ||
             (src_ex_addr != 0u &&
              mem_.TranslateGpuToHost(src_ex_addr) != nullptr));
        ...
        const bool u_base_valid = (u_addr && u_stride)
            ? mem_.TranslateGpuToHost(u_addr) != nullptr : false;
        const bool v_base_valid = (v_addr && v_stride)
            ? mem_.TranslateGpuToHost(v_addr) != nullptr : false;
        const bool dst_base_valid = dst_addr && dst_stride && dst_bpp &&
            mem_.TranslateGpuToHostWrite(dst_addr) != nullptr;
        const bool yuv_planes_valid = !src_yuv ||
            ((src_fmt == 7u || src_fmt == 8u) ||
             ((src_fmt == 17u || src_fmt == 18u) && u_base_valid) ||
             (src_fmt == 15u && u_base_valid && v_base_valid));
        if (!src_base_valid || !dst_base_valid || !yuv_planes_valid) {
            LOG(Trace, "Imx6Gpu%s: VR invalid surface src=%08X/%u fmt=%s "
                       "u=%08X/%u v=%08X/%u dst=%08X/%u fmt=%s",
                mem_.CoreName(), src_addr, src_stride, DeFormatName(src_fmt),
                u_addr, u_stride, v_addr, v_stride,
                dst_addr, dst_stride, DeFormatName(dst_fmt));
            return;
        }
        ...
        const uint32_t step_x = (mode == 0u || mode == 2u)
            ? factor_x : 0x10000u;
        const uint32_t step_y = (mode == 1u || mode == 2u)
            ? factor_y : 0x10000u;
        ...
        const uint32_t src_rot = ((rot_angle >> 8) & 1u)
            ? (rot_angle & 7u)
            : (((src_rot_cfg >> 16) & 1u) ? 4u : 0u);
        const uint32_t dst_rot = ((rot_angle >> 9) & 1u)
            ? ((rot_angle >> 3) & 7u)
            : (((dst_rot_cfg >> 16) & 1u) ? 4u : 0u);
        const uint32_t src_mirror = ((rot_angle >> 15) & 1u)
            ? ((rot_angle >> 12) & 3u) : 0u;
        const uint32_t dst_mirror = ((rot_angle >> 19) & 1u)
            ? ((rot_angle >> 16) & 3u) : 0u;
        const uint32_t src_layout_bpp = (src_fmt == 7u || src_fmt == 8u)
            ? 2u : (src_yuv ? 1u : src_bpp);
        const uint32_t src_linear_pixels_per_row =
            SurfaceWidthFromStride(src_stride, src_layout_bpp, src_layout);
        const uint32_t src_surface_w = (src_rot_cfg & 0xFFFFu)
            ? (src_rot_cfg & 0xFFFFu) : src_linear_pixels_per_row;
        const uint32_t src_surface_h = (mem_.StateReg(kSrcRotHeight) & 0xFFFFu)
            ? (mem_.StateReg(kSrcRotHeight) & 0xFFFFu) : src_bottom;
        const uint32_t dst_surface_w = (dst_rot_cfg & 0xFFFFu)
            ? (dst_rot_cfg & 0xFFFFu)
            : SurfaceWidthFromStride(dst_stride, dst_bpp, dst_layout);
        const uint32_t dst_surface_h = (mem_.StateReg(kDestRotHeight) & 0xFFFFu)
            ? (mem_.StateReg(kDestRotHeight) & 0xFFFFu) : dst_bottom;
        ...
        auto clampCoordinate = [](int32_t value, uint32_t low,
                                  uint32_t high_exclusive) -> uint32_t {
            if (value < static_cast<int32_t>(low))
                return low;
            if (value >= static_cast<int32_t>(high_exclusive))
                return high_exclusive - 1u;
            return static_cast<uint32_t>(value);
        };
        auto sourceCoord = [&](uint32_t x, uint32_t y) -> DeCoord {
            if ((ValidDeRot(src_rot) == 0u && src_mirror == 0u) ||
                src_surface_w == 0u || src_surface_h == 0u)
                return {x, y};
            return TransformDeCoord(x, y, src_surface_w, src_surface_h,
                                    src_rot, src_mirror);
        };
        auto destCoord = [&](uint32_t x, uint32_t y) -> DeCoord {
            if ((ValidDeRot(dst_rot) == 0u && dst_mirror == 0u) ||
                dst_surface_w == 0u || dst_surface_h == 0u)
                return {x, y};
            return TransformDeCoord(x, y, dst_surface_w, dst_surface_h,
                                    dst_rot, dst_mirror);
        };
        ...
        auto readSourcePixel = [&](int32_t x, int32_t y, uint32_t& argb) -> bool {
            ...
            if (p.x >= src_surface_w || p.y >= src_surface_h)
                return false;

            if (src_yuv) {
                return ReadVrYuvPixelGpu(src_addr, src_ex_addr, src_stride,
                                         u_addr, u_stride,
                                         v_addr, v_stride,
                                         p.x, p.y, src_fmt, src_layout,
                                         src_endian, uv_swizzle,
                                         yuv_bt709, argb);
            }

            if ((src_fmt & 0x1Fu) == 6u || (src_fmt & 0x1Fu) == 5u) {
                uint32_t packed = 0u;
                if (!ReadLayoutPackedGpu(src_addr, src_ex_addr, src_stride,
                                         p.x, p.y, 4u, src_layout,
                                         src_endian, packed))
                    return false;
                if ((src_fmt & 0x1Fu) == 5u)
                    packed |= 0xFF000000u;
            ...
            if ((src_fmt & 0x1Fu) == 9u) {
                ...
                if (!ReadLayoutPackedGpu(src_addr, src_ex_addr, src_stride,
                                         ...
                                         src_endian, index))
                    return false;
                argb = mem_.StateReg(kIndexColorTable32 +
                                     (index & 0xFFu) * 4u);
                return true;
            }
            return ReadSurfacePixelGpuLayout(src_addr, src_ex_addr, src_stride,
                                             p.x, p.y, src_fmt, src_swizzle,
                                             src_layout, argb, false,
                                             src_endian);
        };
        auto readDestPixel = [&](uint32_t x, uint32_t y, uint32_t& argb) -> bool {
            const DeCoord p = destCoord(x, y);
            if (p.x >= dst_surface_w || p.y >= dst_surface_h)
                return false;
            if ((dst_fmt & 0x1Fu) == 6u) {
                uint32_t packed = 0u;
                if (!ReadLayoutPackedGpu(dst_addr, 0u, dst_stride,
                                         p.x, p.y, 4u, dst_layout,
                                         dst_endian, packed, false,
                                         MmuClient::PixelEngine))
                    return false;
                argb = ApplyReadSwizzle(packed, dst_swizzle);
                return true;
            }
            return ReadSurfacePixelGpuLayout(dst_addr, 0u, dst_stride,
                                             p.x, p.y, dst_fmt, dst_swizzle,
                                             dst_layout, argb, false,
                                             dst_endian,
                                             MmuClient::PixelEngine);
        };
        auto writeDestPixel = [&](uint32_t x, uint32_t y, uint32_t argb) -> bool {
            ...
            if ((dst_fmt & 0x1Fu) == 6u) {
                ...
                return WriteLayoutPackedGpu(dst_addr, 0u, dst_stride,
                                            p.x, p.y, 4u, dst_layout,
                                            dst_endian, packed);
            }
            return WriteSurfacePixelGpuLayout(dst_addr, 0u, dst_stride,
                                              p.x, p.y, dst_fmt, dst_swizzle,
                                              dst_layout, argb, false,
                                              dst_endian);
        };

        auto loadKernel = [&](uint32_t table_base, int64_t coordinate,
                              int32_t& sample_base, int16_t out[9]) {
            ...
            const uint32_t phase =
                (static_cast<uint32_t>(coordinate) >> 11) & 31u;
            uint32_t row = 0u;
            ...
            for (uint32_t tap = 0; tap < 9u; ++tap) {
                ...
                const uint32_t packed =
                    mem_.StateReg(table_base + (index >> 1) * 4u);
                out[tap] = static_cast<int16_t>((index & 1u)
                    ? (packed >> 16) : (packed & 0xFFFFu));
            }
        };
        auto channel = [](uint32_t argb, uint32_t index) -> uint32_t {
            return (argb >> ((3u - index) * 8u)) & 0xFFu;
        };
        ...
        auto packChannels = [&](const int64_t values[4], uint32_t shift) -> uint32_t {
            ...
            for (uint32_t c = 0; c < 4u; ++c)
                argb |= clamp8(roundShiftSigned(values[c], shift)) <<
                        ((3u - c) * 8u);
            return argb;
        };
        ...
        if (mode == 2u) {
            ...
            if (configured >= 1u && configured <= 9u)
                active_taps = configured;
        }
        ...
        for (uint32_t dy = dst_top; dy < dst_bottom; ++dy) {
            const int64_t fy = origin_y +
                static_cast<int64_t>(dy - dst_top) * step_y;
            for (uint32_t dx = dst_left; dx < dst_right; ++dx) {
                const int64_t fx = origin_x +
                    static_cast<int64_t>(dx - dst_left) * step_x;
                ...
                if (mode == 0u) { /* horizontal */
                    ...
                    for (uint32_t ty = first_tap; ty < last_tap; ++ty) {
                        uint32_t pixel = 0u;
                        if (!readSourcePixel(sample_x,
                                             base_y + static_cast<int32_t>(ty) - 4,
                                             pixel))
                            continue;
                        for (uint32_t c = 0; c < 4u; ++c)
                    ...
                    for (uint32_t ty = first_tap; ty < last_tap; ++ty) {
                        ...
                        for (uint32_t tx = first_tap; tx < last_tap; ++tx) {
                            uint32_t pixel = 0u;
                            if (!readSourcePixel(
                                    base_x + static_cast<int32_t>(tx) - 4,
                                    base_y + static_cast<int32_t>(ty) - 4,
                                    pixel))
                                continue;
                            for (uint32_t c = 0; c < 4u; ++c)
                                horizontal[c] +=
                                    static_cast<int64_t>(channel(pixel, c)) * kx[tx];
                        }
                        for (uint32_t c = 0; c < 4u; ++c)
                ...
                if (alpha_enable) {
                    ...
                    if (readDestPixel(dx, dy, dst_argb))
                        filtered = BlendPePixel(filtered, dst_argb, alpha_control,
                                                alpha_modes, color_multiply_modes,
