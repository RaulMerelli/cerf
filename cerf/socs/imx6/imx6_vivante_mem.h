...
public:
    ...
    bool Mmuv1LooksConfigured(MmuClient client) const { return s_.regs_[Mmuv1PageTableRegister(client) >> 2] != 0u; }

    bool TranslateMmuv1(uint32_t gpu_addr, MmuClient client, uint32_t& phys) const;

    uint32_t ActiveMtlbBase(MmuClient client) const;

    static uint32_t Mmuv2MtlbShift(uint32_t configuration) {
        return (configuration & kMmuv2Mode1k) != 0u ? kMmuv2Mtlb1kShift : kMmuv2Mtlb4kShift;
    }
    ...
    bool WalkMmuv2(uint32_t gpu_addr, bool write, MmuClient client, MmuWalkResult& walk) const;
    bool TranslationCacheMatches(const TranslationPageCache& cache, uint32_t gpu_addr, MmuClient client,
                                 bool write) const;
    void FillTranslationCache(TranslationPageCache& cache, uint32_t gpu_addr, const MmuWalkResult& walk,
                              const uint8_t* read_page, uint8_t* write_page) const;
    void ReportMmuFault(MmuClient client, MmuException exception, uint32_t gpu_addr) const;
