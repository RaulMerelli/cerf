...
public:
    ...
    bool Handles(uint32_t channel, int event) const override {
        /* KTP400's ecspi.dll uses the standard i.MX6 ECSPI3 script pair:
           CHNENBL event 7 drives TX channel 1 and event 8 drives RX channel 2. */
        return (channel == 1u && event == 7) ||
               (channel == 2u && event == 8);
    }

    void Complete(uint32_t channel, uint32_t mode,
                  uint32_t buffer_pa) override {
        ...
        if (channel == 1u) {
            for (uint32_t off = 0; off < bytes; off += 4u) {
                ...
                if (uint8_t* src = memory.TryTranslate(buffer_pa + off))
        ...
        for (uint32_t off = 0; off < bytes; off += 4u) {
            ...
            if (uint8_t* dst = memory.TryTranslateWrite(buffer_pa + off))
