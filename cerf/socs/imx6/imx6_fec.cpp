...
private:
    ...
    uint32_t ReadReg(uint32_t off) {
        ...
        default:
            if ((off >= kRmonBase && off < kRmonBase + 0x80u) || (off >= kIeeeBase && off < kIeeeBase + 0x40u)) {
    ...
    void WriteReg(uint32_t off, uint32_t value) {
        ...
        case kTdar:
            eir_ |= rings_.RequestTransmit(emu_.Get<EmulatedMemory>(), emu_.Get<NetworkBackend>(),
                                           (ecr_ & kEcrEtherEn) != 0u, kCableConnected, etdsr_);
            UpdateIrq();
            return;
        case kEcr:
            if (value & kEcrReset) {
    ...
    uint16_t ReadPhyRegister(uint32_t phy, uint32_t reg) const {
        ...
        case 0x01:
            return kCableConnected ? 0x786Du  /* BMSR: 10/100, autoneg complete, link up */
        ...
        case 0x05:
            return kCableConnected ? 0x45E1u  /* link partner ability + ACK */
                                   : 0x0000u; /* no link partner while cable is unplugged */
        case 0x1F: return 0x0000u;
        /* references/linux/include/uapi/linux/mii.h: MII_CTRL1000 0x09 and
           MII_STAT1000 0x0a are the 1000BASE-T registers, present only when
           BMSR_ESTATEN (0x0100) is set.  BMSR above clears it, so this PHY
           carries neither register. */
        case 0x09:
