...
bool VivanteMem::TranslateMmuv1(uint32_t gpu_addr, MmuClient client, uint32_t& phys) const {
    if (!Mmuv1LooksConfigured(client)) return false;

    if (gpu_addr < kMmuv1GpuMemStart) {
        const uint32_t memory_base = s_.regs_[Mmuv1MemoryBaseRegister(client) >> 2];
    ...
    /* MMUv1 indexes a 2 MiB flat table with GPU VA[30:12].  PTEs are
       ...
       The hmi_ktp400_mobile_v13 WinCE allocator uses only its first 1 MiB.  References:
       etnaviv_iommu.c etnaviv_iommuv1_map and hmi_ktp400_mobile_v13 libGALCore.dll IDA
       sub_EF0086A8/sub_EF0058BC/sub_EF00AC34. */
    const uint32_t page_index = (gpu_addr - kMmuv1GpuMemStart) >> 12;
    const uint32_t table_base = s_.regs_[Mmuv1PageTableRegister(client) >> 2];
    uint32_t pte = 0u;
...
bool VivanteMem::WalkMmuv2(uint32_t gpu_addr, bool write, MmuClient client, MmuWalkResult& walk) const {
    walk = MmuWalkResult{};
    if (!MmuLooksConfigured()) return false;
    ...
    if ((walk.mtlb_desc & kMmuv2PtePresent) == 0u || (walk.mtlb_desc & kMmuv2PteException) != 0u) {
        ReportMmuFault(client, MmuException::SlaveNotPresent, gpu_addr);
        return false;
    }

    const uint32_t page_type = (walk.mtlb_desc & kMmuv2MtlbPageSizeMask) >> 2;
    ...
    const uint32_t stlb_base = walk.mtlb_desc & kMmuv2StlbAddressMask;
...
bool VivanteMem::TranslationCacheMatches(const TranslationPageCache& cache, uint32_t gpu_addr, MmuClient client,
                                         bool write) const {
    ...
    uint32_t pte = 0u;
    if (!ReadPhysicalU32(cache.mtlb_entry_addr, mtlb_desc) || !ReadPhysicalU32(cache.pte_entry_addr, pte)) return false;
    if (mtlb_desc != cache.mtlb_desc || pte != cache.pte || (mtlb_desc & kMmuv2PtePresent) == 0u ||
...
void VivanteMem::FillTranslationCache(TranslationPageCache& cache, uint32_t gpu_addr, const MmuWalkResult& walk,
...
void VivanteMem::ReportMmuFault(MmuClient client, MmuException exception, uint32_t gpu_addr) const {
...
const uint8_t* VivanteMem::TranslateMmuSafeRead(uint32_t fault_addr) const {
    if (!s_.mmu_safe_address_written_) return nullptr;
    const uint32_t safe_base = s_.regs_[kMmuv2SafeAddress >> 2] & ~0x3Fu;
...
const uint8_t* VivanteMem::TranslateGpuToHost(uint32_t gpu_addr, MmuClient client) const {
    auto& mem = emu_.Get<EmulatedMemory>();
    if (MmuLooksConfigured()) {
        const uint32_t slot = static_cast<uint32_t>(client) & 3u;
        const uint32_t offset = gpu_addr & 0xFFFu;
        TranslationPageCache& cache = translation_cache_[slot];
        if (TranslationCacheMatches(cache, gpu_addr, client, false)) return cache.read_page + offset;
        ...
        if (auto* mapped_page = mem.TryTranslate(phys_page)) {
            FillTranslationCache(cache, gpu_addr, walk, mapped_page, mem.TryTranslateWrite(phys_page));
    ...
    if (TranslateMmuv1(gpu_addr, client, mmuv1_phys)) return mem.TryTranslate(mmuv1_phys);
    ...
    if (auto* arm_mmu = emu_.TryGet<ArmMmuProbe>()) {
        ...
        if (arm_mmu->PeekVaToPa(gpu_addr, &pa)) {
            if (auto* mapped = mem.TryTranslate(pa)) return mapped;
        }
    }
    return mem.TryTranslate(gpu_addr);
}

uint8_t* VivanteMem::TranslateGpuToHostWrite(uint32_t gpu_addr, MmuClient client) const {
    auto& mem = emu_.Get<EmulatedMemory>();
    if (MmuLooksConfigured()) {
        const uint32_t slot = static_cast<uint32_t>(client) & 3u;
        const uint32_t offset = gpu_addr & 0xFFFu;
        TranslationPageCache& cache = translation_cache_[slot];
        if (TranslationCacheMatches(cache, gpu_addr, client, true)) return cache.write_page + offset;
        ...
        if (auto* mapped_page = mem.TryTranslateWrite(phys_page)) {
            FillTranslationCache(cache, gpu_addr, walk, mem.TryTranslate(phys_page), mapped_page);
            return mapped_page + offset;
        }
        ReportMmuFault(client, MmuException::OutOfBound, gpu_addr);
        cache.gpu_page = 0xFFFFFFFFu;
    ...
    uint32_t mmuv1_phys = 0u;
    if (TranslateMmuv1(gpu_addr, client, mmuv1_phys)) return mem.TryTranslateWrite(mmuv1_phys);

    if (auto* arm_mmu = emu_.TryGet<ArmMmuProbe>()) {
        uint32_t pa = 0u;
        if (arm_mmu->PeekVaToPa(gpu_addr, &pa)) {
            if (auto* mapped = mem.TryTranslateWrite(pa)) return mapped;
        }
    }
    return mem.TryTranslateWrite(gpu_addr);
}

bool VivanteMem::TranslateGpuViaMmu(uint32_t gpu_addr, bool write, MmuClient client, uint32_t& phys) const {
    MmuWalkResult walk{};
    if (WalkMmuv2(gpu_addr, write, client, walk)) {
        phys = walk.phys;
        return true;
    }
    if (!MmuLooksConfigured()) return TranslateMmuv1(gpu_addr, client, phys);
    return false;
}
