...
bool Iop13xxAtuState::CpuPhysToPciMemBus(uint64_t cpu_pa,
                                         uint32_t size,
                                         uint64_t& pci_bus_addr,
                                         uint32_t* window_idx,
                                         bool require_enabled) const {
    if (!RangeFitsLow32(cpu_pa, size)) return false;
    if (require_enabled && (!OutboundGloballyEnabled() ||
                            ((atucmd_ & kAtucmdMemorySpaceEnable) == 0) ||
                            !BusMasterEnabled())) return false;
    ...
    for (uint32_t i = 0; i < kMemoryWindowCount; ++i) {
        ...
        pci_bus_addr = (static_cast<uint64_t>(w.wtvr) << 32) |
                       static_cast<uint32_t>(cpu_pa);
...
bool Iop13xxAtuState::PciMemBusToCpuPhys(uint64_t pci_bus_addr,
                                         uint32_t size,
                                         uint64_t& cpu_pa,
                                         uint32_t* window_idx,
                                         bool require_enabled) const {
    ...
    for (uint32_t i = 0; i < kMemoryWindowCount; ++i) {
        ...
        cpu_pa = (static_cast<uint64_t>(w.bar & kOutboundWindowSectionMask) << 32) |
                 pci_lo;
        if (window_idx) *window_idx = i;
        return true;
    }
...
bool Iop13xxAtuState::CpuPhysToPciIoBus(uint64_t cpu_pa,
                                        uint32_t size,
                                        uint32_t& pci_io_addr,
                                        bool require_enabled) const {
    ...
    pci_io_addr = (oiowtvr_ & 0xFFFF0000u) |
                  static_cast<uint32_t>(cpu_pa & 0xFFFFu);
    return true;
}

bool Iop13xxAtuState::PciIoBusToCpuPhys(uint32_t pci_io_addr,
                                        uint32_t size,
                                        uint64_t& cpu_pa,
                                        bool require_enabled) const {
    ...
    cpu_pa = static_cast<uint64_t>(oiobar_ & 0xFFFF0000u) |
             static_cast<uint64_t>(pci_io_addr & 0xFFFFu);
    return true;
}
