...
constexpr uint32_t kIcrKnownMask = kIcrFastMode | kIcrUnitReset | kIcrSadIe |
    kIcrAldIe | kIcrSsdIe | kIcrBerrIe | kIcrRxFullIe | kIcrTxEmptyIe |
    kIcrGcd | kIcrUe | kIcrSclEn | kIcrMAbort | kIcrTByte | kIcrNack |
    kIcrMStop | kIcrMStart;
...
public:
    ...
    void OnShutdown() override {
        if (auto* device = emu_.TryGet<Iop13xxI2cDevice>())
            device->Stop();
    }
    ...
    void WriteHalf(uint32_t addr, uint16_t value) override {
        ...
        WriteWordRaw(aligned, (ReadWordRaw(aligned) & ~mask) |
    ...
    void RestoreState(StateReader& reader) override {
        ...
        if (has_device)
            emu_.Get<Iop13xxI2cDevice>().RestoreState(reader);
...
private:
    uint32_t ReadWordRaw(uint32_t addr) {
        ...
        default:
            HaltUnsupportedAccess("IOP13xx I2C register read", addr, 0);
        }
    }
    ...
    void WriteWordRaw(uint32_t addr, uint32_t value) {
        ...
        case kBmrOffset:
            return;
        default:
            HaltUnsupportedAccess("IOP13xx I2C register write", addr, value);
        }
    }
    ...
    void WriteControl(uint32_t value) {
        ...
        if (control & kIcrTByte) {
            isr_ &= ~(kIsrRxFull | kIsrTxEmpty | kIsrNack | kIsrBerrd |
                      kIsrBBusy | kIsrUnitBusy);
            TransferByte(control);
        }
        if (control & kIcrMStop)
            StopTransaction();
    }
    ...
    void TransferByte(uint32_t control) {
        ...
        if (device_selected_ && device) {
            if (!device->WriteByte(static_cast<uint8_t>(idbr_ & 0xFFu)))
                isr_ |= kIsrNack;
            isr_ |= kIsrTxEmpty;
            return;
        }
    ...
    void StartAddress(uint8_t address_byte) {
        ...
        isr_ = (isr_ & ~(kIsrRxFull | kIsrTxEmpty | kIsrNack | kIsrRxRead)) |
               kIsrTxEmpty;
        if (device_read_)
            isr_ |= kIsrRxRead;
        auto* device = emu_.TryGet<Iop13xxI2cDevice>();
