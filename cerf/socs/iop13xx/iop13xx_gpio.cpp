...
public:
    ...
    uint32_t ReadWord(uint32_t addr) override {
        ...
        case 0x04u:
            if (auto* input = emu_.TryGet<Iop13xxGpioInput>())
                return input->ReadPins();
            HaltUnsupportedAccess("IOP13xx GPIO input provider", addr, 0);
        case 0x08u:
            return interrupt_enable_;
        default:
            HaltUnsupportedAccess("IOP13xx GPIO register read", addr, 0);
        }
    }
    ...
    void WriteWord(uint32_t addr, uint32_t value) override {
        ...
        case 0x08u:
            ...
            return;
        default:
            HaltUnsupportedAccess("IOP13xx GPIO register write", addr, value);
        }
    }
