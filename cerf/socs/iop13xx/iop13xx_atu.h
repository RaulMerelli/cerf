...
/* Shared IOP13xx ATU state and pure translation helpers.

   This service does not register an MMIO window and does not dispatch PCI
   cycles by itself.  iop13xx_pmmr.cpp owns the guest-visible registers and
   stores their values here.  The next step's outbound bridge will consume the
   same helpers to translate internal-bus physical addresses into PCI bus
   memory cycles. */
...
public:
    ...
    bool CpuPhysToPciMemBus(uint64_t cpu_pa,
                            uint32_t size,
                            uint64_t& pci_bus_addr,
                            uint32_t* window_idx = nullptr,
                            bool require_enabled = true) const;

    bool PciMemBusToCpuPhys(uint64_t pci_bus_addr,
                            uint32_t size,
                            uint64_t& cpu_pa,
                            uint32_t* window_idx = nullptr,
                            bool require_enabled = true) const;

    bool CpuPhysToPciIoBus(uint64_t cpu_pa,
                           uint32_t size,
                           uint32_t& pci_io_addr,
                           bool require_enabled = true) const;

    bool PciIoBusToCpuPhys(uint32_t pci_io_addr,
                           uint32_t size,
                           uint64_t& cpu_pa,
                           bool require_enabled = true) const;
