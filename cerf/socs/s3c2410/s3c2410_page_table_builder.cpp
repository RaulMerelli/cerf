...
uint32_t S3C2410PageTableBuilder::VaToPa(uint32_t va) const {
    for (const auto& e : kOat) {
        if (va >= e.va_base && va < e.va_base + e.size) {
            return e.pa_base + (va - e.va_base);
        }
    }
    LOG(Caution, "S3C2410PageTableBuilder::VaToPa: VA 0x%08X is "
...
std::vector<DramRegion> S3C2410PageTableBuilder::CachedDramRegions() const {
...
std::vector<BackedRegion> S3C2410PageTableBuilder::BackedMemoryRegions() const {
