...
inline constexpr uint32_t kSm501PciDeviceVendorDword =
    (kSm501PciDeviceId << 16) | kSm501PciVendorId;
...
/* smibase.dll sub_2B51544 stores the BAR1 config resource and sub_2B5456C
   returns it through IOCTL 11.  VGXaudio passes that value to CEDDK
   TransBusAddrToStatic; MP377 nk.exe sub_80447F90/sub_80448ACC leaves PCI
...
inline constexpr uint32_t kSm501PciRegsBarDword =
    kSm501RegsCpuStaticPa | kSm501PciRegsBarFlags;
...
public:
    ...
    uint32_t Read(uint32_t pa) const;
    void Write(uint32_t pa, uint32_t value);
