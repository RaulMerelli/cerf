...
public:
    ...
    static bool IsCommandRegister(uint32_t offset) {
        return offset == 0x10000Cu;
    }
    static bool IsDataPort(uint32_t offset) {
        return offset >= 0x110000u && offset < 0x110100u;
    }
...
private:
    ...
    template <typename T>
    static void WriteVectorState(StateWriter& w, const std::vector<T>& v) {
        const uint64_t n = static_cast<uint64_t>(v.size());
        w.Write(n);
        if (n) w.WriteBytes(v.data(), static_cast<size_t>(n * sizeof(T)));
    }
    template <typename T>
    void ReadVectorState(StateReader& r, std::vector<T>& v,
                         size_t max_expected, const char* what) {
        uint64_t n = 0;
        r.Read(n);
        if (n > static_cast<uint64_t>(max_expected)) {
            LOG(Caution, "%s: count=%llu maximum=%zu\n", what,
                static_cast<unsigned long long>(n), max_expected);
    ...
    static uint16_t ApplyDdiRop16(uint8_t rop, uint16_t s, uint16_t d,
                                  uint16_t p = 0xFFFFu);
    VramBlitRect NormalizeVramBlitRect(const State2d& st) const;
    void ExecuteDdiVideoToVideoChunk(const State2d& st, const VramBlitRect& r,
                                     uint32_t y_off, uint32_t rows);
    void ExecuteDdiVideoToVideo(const State2d& st);
    void ExecuteDdiVgx2dCommand(uint32_t cmd);

    SiemensMp377Sm501Regs* registers_ = nullptr;
