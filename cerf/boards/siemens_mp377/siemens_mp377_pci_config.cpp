...
public:
    ...
    uint32_t ReadPrimary(uint32_t occar) override {
        ...
        if (select == kSm501Select)
            return reg < sm501_cfg_.size() ? sm501_cfg_[reg] : 0xFFFFFFFFu;
        if (select == kErtec400Select)
            return reg < ertec_cfg_.size() ? ertec_cfg_[reg] : 0xFFFFFFFFu;
        return 0xFFFFFFFFu;
    }
    ...
    bool WritePrimary(uint32_t occar, uint32_t value) override {
        ...
        if (select != kSm501Select || reg >= sm501_cfg_.size())
            return true;
    ...
    uint32_t ReadSecondary(uint32_t occar) override {
        const uint32_t reg = (occar & 0xFFu) >> 2;
        if (DeviceSelect(occar) != kSecondarySm501Select ||
            reg >= sm501_cfg_.size()) {
            return 0xFFFFFFFFu;
        }
...
private:
    ...
    void BuildErtec400Config() {
        ertec_cfg_.fill(0u);
        /* eddertec400.dll sub_28E1CA8 PCI identity and BAR layout. */
    ...
    void WriteErtec400Config(uint32_t reg, uint32_t value) {
        ...
        switch (reg) {
            ...
            case 0x01:
            case 0x03:
                ertec_cfg_[reg] = (ertec_cfg_[reg] & 0xFFFF0000u) |
                                  (value & 0x0000FFFFu);
                return;
            case 0x0C:
            case 0x0D:
                ...
                return;
            case 0x0F:
                ertec_cfg_[reg] = (ertec_cfg_[reg] & 0xFFFFFF00u) |
                                  (value & 0x000000FFu);
                return;
            default:
                return;
    ...
    bool WriteSm501Config(uint32_t reg, uint32_t value) {
        ...
        case 0x0E:
            return true;
        case 0x01:
            sm501_cfg_[reg] = (sm501_cfg_[reg] & 0xFFFF0000u) |
                              (value & 0x0000FFFFu);
            return true;
        case 0x03:
            sm501_cfg_[reg] = (sm501_cfg_[reg] & 0xFFFF0000u) |
        ...
        case 0x0C:
            ...
            return true;
        case 0x0F:
            sm501_cfg_[reg] = (sm501_cfg_[reg] & 0xFFFFFF00u) |
                              (value & 0x000000FFu);
            return true;
        default:
            return false;
