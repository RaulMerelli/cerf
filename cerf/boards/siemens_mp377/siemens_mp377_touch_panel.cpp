...
static constexpr TouchCalibrationProfile kTouchCalibrationProfiles[] = {
    { SiemensMp377PanelProfile::Inch12_800x600,
      { {1855, 1859}, {3023, 3008}, {3034,  676}, { 672,  672}, { 677, 3011} } },
    { SiemensMp377PanelProfile::Inch15_1024x768,
      { {1879, 1844}, {3173, 3073}, {3175,  655}, { 585,  654}, { 586, 3035} } },
    { SiemensMp377PanelProfile::Inch19_1280x1024,
      { {1842, 1838}, {2937, 2976}, {2936,  717}, { 697,  713}, { 712, 2994} } },
};

const TouchCalibrationProfile& CurrentTouchCalibrationProfile() {
    for (const auto& profile : kTouchCalibrationProfiles) {
        if (profile.profile == kMp377HwiPanelProfile)
            return profile;
    }
...
TouchAffineInverse BuildTouchAffineInverse(const TouchCalibrationProfile& profile) {
    ...
    double x_coeff[3] = {};
    double y_coeff[3] = {};
    if (!Solve3x3(ata, atx, x_coeff) || !Solve3x3(ata, aty, y_coeff))
...
void SiemensMp377TouchPanel::OnReady() {
    const TouchAffineInverse affine =
        BuildTouchAffineInverse(CurrentTouchCalibrationProfile());
...
uint16_t SiemensMp377TouchPanel::PopSmiCommand() {
    if (smi_cmd_head_ == smi_cmd_tail_)
        return static_cast<uint16_t>(smi_last_cmd_.load(std::memory_order_relaxed));
...
void SiemensMp377TouchPanel::HostPointToTouchRaw(
    uint32_t x, uint32_t y, uint16_t* raw_x, uint16_t* raw_y) const {
    /* touch.dll owns the GWES calibration step.  The emulated hardware must
       ...
       points come from touch.dll sub_29E2474: center, then 10%/90% corners. */
    const double dx = static_cast<double>(x);
    const double dy = static_cast<double>(y);
    *raw_x = ClampAdc12(touch_affine_[0] * dx + touch_affine_[1] * dy +
                        touch_affine_[2]);
    *raw_y = ClampAdc12(touch_affine_[3] * dx + touch_affine_[4] * dy +
                        touch_affine_[5]);
}
