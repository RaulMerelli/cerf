...
void SiemensMp377Sm501Ac97::UpdateStatus() {
    ...
    bclk_running_ = gate_on && enabled && !reset && !sync_stopped && link_powered && pins_routed &&
                    board_reset_released && codec_ready_;
    Registers().regs_[kRxSlot0TagReg / 4u] = rx_slot0_ = (codec_ready_ && link_powered) ? kTagValidFrame : 0u;
    Registers().regs_[kControlStatusReg / 4u] = ControlStatusValue();
}

void SiemensMp377Sm501Ac97::ResetCodec(bool cold) {
    for (uint16_t& r : codec_registers_)
        r = 0;
...
void SiemensMp377Sm501Ac97::ProcessFrame() {
    ...
    const bool have_cmd = (tx_dirty_mask_ & (kDirtySlot1 | kDirtySlot2)) == (kDirtySlot1 | kDirtySlot2);
    const bool want_pcm = (tx_slot0_ & (kTagSlot3Valid | kTagSlot4Valid)) == (kTagSlot3Valid | kTagSlot4Valid);
    const bool have_pcm = (tx_dirty_mask_ & (kDirtySlot3 | kDirtySlot4)) == (kDirtySlot3 | kDirtySlot4);
...
void SiemensMp377Sm501Ac97::ClearInterrupt() {
    auto& registers = Registers();
    if (!irq_pending_ && (registers.regs_[kSm501IrqStatusReg / 4u] & kSm501Ac97IrqBit) == 0u) return;
    irq_pending_ = false;
...
void SiemensMp377Sm501Ac97::WriteCodecRegister(uint32_t reg, uint16_t value) {
    ...
    switch (reg) {
    case 0x02u:
    case 0x04u: value &= 0xBF3Fu; break;
    case 0x06u: value &= 0x801Fu; break;
    case 0x0Au: value &= 0x801Eu; break;
    case 0x0Cu:
    case 0x0Eu: value &= 0x801Fu; break;
    case 0x10u:
    case 0x12u:
    case 0x14u:
    case 0x16u:
    case 0x18u: value &= 0x9F1Fu; break;
    case 0x1Au: value &= 0x0707u; break;
    case 0x1Cu: value &= 0x8F0Fu; break;
    case 0x20u: value &= 0x3F00u; break;
