...
public:
    ...
    void RenderInto(uint32_t* dib, uint32_t host_w, uint32_t host_h) override {
        auto* video = emu_.TryGet<siemens_mp377::SiemensMp377Sm501Video>();
        if (!dib || host_w == 0 || host_h == 0 || !video) return;
        ...
        const uint32_t max_h_by_vram =
            (visible_off < kSm501FbBytes && visible_pitch) ? ((kSm501FbBytes - visible_off) / visible_pitch) : 0u;
    ...
    void PresentedSize(uint32_t& w, uint32_t& h) override {
        w = 0u;
        h = 0u;
        if (auto* video = emu_.TryGet<siemens_mp377::SiemensMp377Sm501Video>()) {
            w = video->PanelWidth();
            h = video->PanelHeight();
        }
        /* The panel-timing registers are Undefined until the guest programs
           them; until then the bootloader-declared panel is what is on screen. */
