...
/* siemens_mp377_v1040 eddertec400.dll EDDDeviceOpen selects these ERTEC400 conversions:
...
uint32_t AsicAddressToPhysical(uint32_t address) {
    if ((address & 0xE0000000u) == 0x20000000u) return kDmaPhysicalBase + (address & 0x1FFFFFFFu);
...
struct SiemensMp377Ertec400Nrt::Impl {
    ...
    uint8_t* Descriptor(uint32_t asic_address) {
        return emu.Get<EmulatedMemory>().TryTranslateWrite(AsicAddressToPhysical(asic_address));
    }
...
void SiemensMp377Ertec400Nrt::OnReady() {
    emu_.Get<NetworkBackend>().SetReceiveCallback(
        [this](const uint8_t* frame, std::size_t length) { impl_->Receive(frame, length); });
