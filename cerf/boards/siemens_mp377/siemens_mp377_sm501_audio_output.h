...
// MP377 board audio sink and the host-side pacer for SM501 firmware buffers.
// SM501 Databook ch. 11 defines the fixed AC-link cadence; VGXaudio.dll
// sub_2987AAC defines the board firmware's two 0x600-byte buffer ABI.
...
private:
    ...
    void CaptureBytes(const uint8_t* host, uint32_t bytes,
                      uint32_t nonzero_samples, uint32_t peak_abs);
