...
/* OEMAddressTable from MP377 nk.exe at VA 0x80409F00.
...
constexpr OatEntry kOat[] = {
    { 0x80000000u, 0x00000000u, MB(256), OatKind::CachedDram },
...
/* Reconstructed non-OAT path: nk.exe sub_804420B8 / OALPAtoVA(pa, cached)
...
public:
    ...
    std::vector<StaticRuntimeAlias> StaticRuntimeAliases() const override;
};
...
SiemensMp377PageTableBuilder::BackedMemoryRegions() const {
    ...
    /* MP377 S7pbhmix.dll sub_2A530D0 maps PA 0xD0140000 and immediately
    ...
    regions.push_back({ kMp377Aspc2RamVa, kMp377Aspc2RamPa,
                        kMp377Aspc2RamSize, PAGE_READWRITE });
    return regions;
}
...
std::vector<StaticRuntimeAlias>
SiemensMp377PageTableBuilder::StaticRuntimeAliases() const {
    std::vector<StaticRuntimeAlias> regions;
    for (const auto& e : kOat) {
        /* nk.exe sub_804420B8 / OALPAtoVA walks OEMAddressTable and ORs
           0x20000000 into the VA for an uncached mapping.  sub_8040FC70 /
           NKCreateStaticMapping returns that VA without installing a PTE. */
        regions.push_back({e.va_base | kOalUncachedAliasBit,
                           e.pa_base, e.size});
    }
    return regions;
}
