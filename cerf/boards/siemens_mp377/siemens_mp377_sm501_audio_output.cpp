...
bool SiemensMp377Sm501AudioOutput::IsActive() const { return active_; }

uint64_t SiemensMp377Sm501AudioOutput::FramesEmitted() const {
    return frames_emitted_total_;
}

void SiemensMp377Sm501AudioOutput::QueueAc97PcmSample(
    uint32_t left20, uint32_t right20) {
    if (!active_) return;
    (void)left20;
    (void)right20;
...
void SiemensMp377Sm501AudioOutput::SetPlaybackEnabled(bool enabled) {
    ...
    if (enabled && !active_) {
        ...
        if (QueuedBlocks() == 0u &&
            (regs.regs_[kSm501IrqStatusReg / 4u] &
             SiemensMp377Sm501AudioMcu::kOutputIrqBit) == 0u) {
...
bool SiemensMp377Sm501AudioOutput::ConsumeIrqBit(uint8_t buffer_bit) {
    ...
    for (uint8_t i = 0u; i < irq_fifo_count_; ++i) {
        ...
        const uint8_t tail = static_cast<uint8_t>(
            (irq_fifo_head_ + irq_fifo_count_ - 1u) % capacity);
        irq_fifo_[tail] = 0u;
        --irq_fifo_count_;
...
void SiemensMp377Sm501AudioOutput::RestoreState(StateReader& r) {
    r.Read(active_);
    {
        ...
        if (irq_fifo_count_ > sizeof(irq_fifo_))
            irq_fifo_count_ = static_cast<uint8_t>(sizeof(irq_fifo_));
        block_queued_ = irq_fifo_count_ != 0u;
    }
    {
        std::lock_guard<std::mutex> lock(pcm_mutex_);
...
void SiemensMp377Sm501AudioOutput::QueueBufferData(
    uint32_t base, uint8_t buffer_bit) {
    auto& regs = emu_.Get<SiemensMp377Sm501Regs>();
    ...
    for (uint32_t i = 0u; i + 7u < kBufferBytes; i += 8u) {
        ...
        const uint32_t left_abs = left < 0
            ? static_cast<uint32_t>(-static_cast<int32_t>(left))
            : static_cast<uint32_t>(left);
        const uint32_t right_abs = right < 0
            ? static_cast<uint32_t>(-static_cast<int32_t>(right))
...
void SiemensMp377Sm501AudioOutput::CaptureBytes(
    const uint8_t* host, uint32_t bytes, uint32_t nonzero_samples,
    uint32_t peak_abs) {
    const bool first_nonzero =
        capture_nonzero_samples_ == 0u && nonzero_samples != 0u;
...
void SiemensMp377Sm501AudioOutput::WriteCaptureWav() {
    ...
    if (!out) {
        LOG(Caution, "MP377 SM501 audio capture: failed_open path=%s bytes=%u\n",
            path, static_cast<uint32_t>(capture_pcm_.size()));
        return;
    }
...
void SiemensMp377Sm501AudioOutput::PushIrqBit(uint8_t buffer_bit) {
    ...
    if (irq_fifo_count_ >= capacity) {
        ...
        const uint8_t tail = static_cast<uint8_t>(
            (irq_fifo_head_ + irq_fifo_count_) % capacity);
...
void SiemensMp377Sm501AudioOutput::ClearQueue() {
    ...
    block_queued_ = false;
    id_pending_consume_.store(false);
    for (uint8_t& bit : irq_fifo_) bit = 0u;
}

std::chrono::microseconds SiemensMp377Sm501AudioOutput::BlockPeriod() const {
    constexpr uint64_t usec =
        (static_cast<uint64_t>(kFramesPerFirmwareHalf) * 1000000ull +
         kDefaultRateHz / 2u) / kDefaultRateHz;
...
void SiemensMp377Sm501AudioOutput::ServiceCompletions() {
    auto& regs = emu_.Get<SiemensMp377Sm501Regs>();
    if (!active_) return;
    if ((regs.regs_[kSm501IrqStatusReg / 4u] &
         SiemensMp377Sm501AudioMcu::kOutputIrqBit) != 0u) return;
