...
/* The BSP registers an IRQ-24 handler (siemens_mp377_v1040, nk.exe OAL IRQ
   dispatch table[24]=0x80443B78 -> sub_80445A70): it reads the bridge struct
   at kernel VA 0x81AAF7EC — mask at +4, status at +8 — and returns
   SYSINTR 47 for status&mask&0x400 (audio), SYSINTR 46 for status&mask!=0
   (touch/SMI).  Window rel 4/8 decode below serves exactly those two. */
...
uint32_t SiemensMp377SmiBridge::Read(uint32_t pa) const {
    switch ((pa & ~3u) & 0x0Fu) {
    case 0x04u: return bridge_enable_.load(std::memory_order_acquire);
    case 0x08u: return bridge_status_.load(std::memory_order_acquire);
    default: return 0u;
    }
}
...
void SiemensMp377SmiBridge::Write(uint32_t pa, uint32_t value) {
    switch ((pa & ~3u) & 0x0Fu) {
    case 0x04u: bridge_enable_.store(value, std::memory_order_release); break;
    case 0x08u:
...
void SiemensMp377SmiBridgeWindow::WriteWord(uint32_t a, uint32_t v) {
    ...
    const uint32_t rel = RelativeOffset(a);
    /* The C410 window overlaps the early console framebuffer: word stores at
       non-register offsets are console pixels, not bridge traffic. */
    if (!IsSupportedOffset(rel)) {
        if (IsC410ConsoleAlias()) return;
...
bool SiemensMp377SmiBridgeWindow::IsC410ConsoleAlias() const {
    return false;
}
