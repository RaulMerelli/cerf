...
public:
    ...
    void OnControlWrite(uint32_t reg_off, uint32_t value) override {
        ...
        if (box_id_injected_ || reg_off != 0x84u || (value & 0x20u) == 0u) return;
        const uint8_t id[] = {0x00u, 0xA0u};
        emu_.Get<Imx6Uart2>().InjectRxPolled(id, sizeof(id));
    ...
    void OnGuestTx(uint8_t byte) override {
        ...
        if (rx_payload_.size() < 64u) rx_payload_.push_back(byte);
    }

private:
    void ReplyToMicroOmsFrame() {
        if (rx_payload_.empty()) return;

        std::vector<uint8_t> reply;
        if (rx_payload_.size() >= 3u && rx_payload_[0] == 0x4Au && rx_payload_[1] == 0x55u && rx_payload_[2] == 0x0Fu) {
