...
void SdCard::SetKtp400HardwareMac(const std::array<uint8_t, 6>& mac) {
    media_.SetHardwareMac(mac);
}
...
void SdCard::BuildCidCsd() {
    ...
    cid_[1]  = 'C';  cid_[2] = 'E';  /* OID */
    cid_[3]  = 'C';  cid_[4] = 'E'; cid_[5] = 'R'; cid_[6] = 'F'; cid_[7] = '0';
    cid_[8]  = 0x10;                 /* product revision 1.0 */
    cid_[9]  = 0x00; cid_[10] = 0x00; cid_[11] = 0x00; cid_[12] = 0x01; /* serial */
    cid_[13] = 0x01; cid_[14] = 0x40; /* mfg date */
    ...
    const uint32_t c_size = static_cast<uint32_t>(
        std::min<uint64_t>((blocks >> (c_size_mult + 2u)) - 1u, 0x0FFFu));
...
void SdCard::ApplyMmcSwitch(uint32_t arg) {
    ...
    switch (access) {
    case 0:                         /* switch command set: accepted, no remap */
        break;
    case 1:                         /* set bits */
        ext_csd_[index] = static_cast<uint8_t>(ext_csd_[index] | value);
        break;
    case 2:                         /* clear bits */
        ext_csd_[index] = static_cast<uint8_t>(ext_csd_[index] & ~value);
        break;
    case 3:                         /* write byte */
        ext_csd_[index] = value;
        break;
    default:
        break;
    }
...
uint32_t SdCard::Status() const {
    ...
    return card_status_ | (ready_for_data ? (1u << 8) : 0u) | (current_state << 9);
}

SdCard::CommandResult SdCard::Command(uint8_t index, uint32_t arg, bool app_cmd) {
    CommandResult r;
    auto r1 = [&] { r.rsp = Rsp::R1; r.resp[0] = Status(); };
    ...
    if (app_cmd) {
        ...
        case 6:                     /* ACMD6 SET_BUS_WIDTH -> R1 */
            r1();
            return r;
        case 42:                    /* ACMD42 SET_CLR_CARD_DETECT -> R1 */
            r1();
            return r;
    ...
    case 3:                         /* SD SEND_RELATIVE_ADDR / MMC SET_RCA */
        ...
        state_ = State::Stby;
        return r;
    case 5:                         /* MMC CMD5 SLEEP/AWAKE; SDIO CMD5 remains unsupported */
        if (mmc_mode_) {
            const bool sleep = ((arg >> 15) & 1u) != 0u;
            if (sleep)
                state_ = State::Stby;  /* no sleep state in this minimal model */
            r.rsp = Rsp::R1b;
            r.resp[0] = Status();
            return r;
        }
    ...
    case 18:                        /* CMD18 READ_MULTIPLE_BLOCK -> R1 + read */
        r1();
        r.starts_read = true;
        xfer_addr_ = high_capacity_ ? (static_cast<uint64_t>(arg) * 512u)
                                    : static_cast<uint64_t>(arg);
        if (mmc_mode_ && index != 18u)
            mmc_predefined_block_count_ = 0;
        state_ = State::Data;
        return r;
    ...
    case 25:                        /* CMD25 WRITE_MULTIPLE_BLOCK -> R1 + write */
        ...
        xfer_addr_ = high_capacity_ ? (static_cast<uint64_t>(arg) * 512u)
                                    : static_cast<uint64_t>(arg);
        if (mmc_mode_ && index != 25u)
            mmc_predefined_block_count_ = 0;
        state_ = State::Rcv;
        return r;
    case 35:                        /* CMD35 ERASE_GROUP_START (MMC) */
        erase_start_addr_ = high_capacity_ ? (static_cast<uint64_t>(arg) * 512u)
                                           : static_cast<uint64_t>(arg);
        erase_start_valid_ = true;
        r1();
        return r;
    case 36:                        /* CMD36 ERASE_GROUP_END (MMC) */
        erase_end_addr_ = high_capacity_ ? (static_cast<uint64_t>(arg) * 512u)
                                         : static_cast<uint64_t>(arg);
        erase_end_valid_ = true;
        r1();
        return r;
    case 38: {                      /* CMD38 ERASE */
        if (erase_start_valid_ && erase_end_valid_)
            media_.Erase(erase_start_addr_, erase_end_addr_);
...
void SdCard::ReadBlock(uint8_t* dst512) {
    ...
    if (xfer_switch_status_) {       /* CMD6: 512-bit switch-function status */
        ...
        dst512[0]  = 0x00; dst512[1]  = 0x00; dst512[2]  = 0x00; dst512[3]  = 0x03;
        dst512[13] = 0x80;           /* data structure version 1.0 */
        dst512[16] = 0x00;           /* current function: default speed */
        xfer_switch_status_ = false;
        return;
    }
    media_.Read(xfer_addr_, dst512);
    xfer_addr_ += 512u;
    if (mmc_predefined_block_count_ != 0)
        --mmc_predefined_block_count_;
    state_ = State::Tran;
}

void SdCard::WriteBlock(const uint8_t* src512) {
    media_.Write(xfer_addr_, src512);
    xfer_addr_ += 512u;
    if (mmc_predefined_block_count_ != 0)
        --mmc_predefined_block_count_;
    state_ = State::Tran;
