...
Ktp400BackingLayout InspectKtp400Backing(const std::vector<uint8_t>& data) {
    ...
    if ((out.part_type != 0x0Bu && out.part_type != 0x0Cu) || out.part_lba == 0u || out.part_blocks == 0u) return out;
    const uint64_t bpb_off = uint64_t(out.part_lba) * 512u;
    if (bpb_off + 512u > data.size() || uint64_t(out.part_lba) + out.part_blocks > data.size() / 512u) return out;
    ...
    if (out.bytes_per_sector != 512u || out.sectors_per_cluster == 0u || bpb[16] == 0u || Get32(bpb + 36u) == 0u ||
...
void SdCardMedia::ConfigureKtp400(const std::string& device_dir, const std::string& container_name,
                                  KtpMobileOpType op_type, KtpMobilePanel panel, const std::array<uint8_t, 6>& mac) {
    ...
    op_type_ = op_type;
    panel_ = panel;
    ktp400_configured_ = true;
}

void SdCardMedia::InitializeMmcLayout() {
    if (layout_initialized_ || !ktp400_configured_) return;
...
void SdCardMedia::EnsureKtp400PdcfsRootDirs() {
    ktp_mobile_emmc::EnsurePdcfsLayout(
        data_, [this](uint64_t offset, uint64_t length) { PersistBackingRange(offset, length); },
        fwf_fsf::ParseFsfVolume(fwf_container_));
}

void SdCardMedia::EnsureKtp400HardwareInfoSeed() {
    ktp_mobile_emmc::EnsureFactoryLayout(data_, fwf_container_, hardware_mac_, op_type_, panel_);
}

void SdCardMedia::PersistBackingRange(uint64_t offset, uint64_t length) {
    if (!persistent_ || offset >= data_.size() || length == 0u) return;

    const uint64_t end = std::min<uint64_t>(offset + length, data_.size());
    if (end <= offset) return;

    MarkDirtyBackingRange(offset, end - offset);
    if (dirty_bytes_pending_ < (4ull * 1024ull * 1024ull)) return;

    FlushDirtyBackingRanges();
}

void SdCardMedia::MarkDirtyBackingRange(uint64_t offset, uint64_t length) {
    if (offset >= data_.size() || length == 0u) return;
...
void SdCardMedia::FlushDirtyBackingRanges() {
    if (dirty_ranges_.empty() || data_.empty()) return;

    std::fstream out(backing_path_, std::ios::binary | std::ios::in | std::ios::out);
    if (!out.good()) {
        ...
        create.close();
        out.open(backing_path_, std::ios::binary | std::ios::in | std::ios::out);
    }
    if (!out.good()) return;
    ...
    for (const auto& r : dirty_ranges_) {
        ...
        out.write(reinterpret_cast<const char*>(data_.data() + begin), static_cast<std::streamsize>(end - begin));
    }
    dirty_ranges_.clear();
...
void SdCardMedia::PersistKtp400HardwareInfoSeed() {
    ...
    PersistBackingRange(kKtp400FactoryTableOff, 0x00006000u + kKtp400PartLbaBytes);

    /* Persist the compact installed Firmware root and its linear-store
       envelope.  It can span more than one sector. */
    std::vector<uint8_t> installed_firmware;
    if (cerf::fwf_oms::ExtractInstalledFirmwareSummary(fwf_container_.data(), fwf_container_.size(),
                                                       installed_firmware)) {
        PersistBackingRange(kKtp400FwfInfoOff,
                            static_cast<uint64_t>(installed_firmware.size()) + 8u + kKtp400PartLbaBytes);
    }
}
