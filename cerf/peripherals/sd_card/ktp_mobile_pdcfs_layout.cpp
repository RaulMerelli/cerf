...
void EnsurePdcfsLayout(std::vector<uint8_t>& data, const PersistRange& persist_range,
    ...
    const auto cluster_off = [&](uint32_t clus) -> uint64_t { return dataoff + uint64_t(clus - 2u) * spc * 512u; };
    ...
    const auto has_entry = [&](const char name[11]) {
        for (uint32_t off = 0; off + 32u <= root_bytes; off += 32u) {
            if (root[off] == 0x00u) return false;
            if (root[off] == 0xE5u) continue;
            if ((root[off + 11u] & 0x0Fu) == 0x0Fu) continue;
            if (std::memcmp(root + off, name, 11u) == 0) return true;
        }
        return false;
    };
    const auto find_short_entry = [&](const char name[11]) -> uint8_t* {
        for (uint32_t off = 0; off + 32u <= root_bytes; off += 32u) {
            if (root[off] == 0x00u) return nullptr;
            if (root[off] == 0xE5u) continue;
            if ((root[off + 11u] & 0x0Fu) == 0x0Fu) continue;
            if (std::memcmp(root + off, name, 11u) == 0) return root + off;
        }
        return nullptr;
    };
    const auto alloc_entry_run = [&](uint32_t count) -> uint8_t* {
        ...
        for (uint32_t off = 0; off + 32u <= root_bytes; off += 32u) {
            if (root[off] == 0x00u || root[off] == 0xE5u) {
                ++run;
                if (run == count) return root + off + 32u - count * 32u;
            } else {
                run = 0;
            }
        }
    ...
    const auto alloc_cluster = [&]() -> uint32_t {
        const uint32_t total_clusters = static_cast<uint32_t>((data.size() - dataoff) / (uint64_t(spc) * 512u));
        for (uint32_t clus = 2u; clus < total_clusters + 2u; ++clus) {
            if (get_fat(clus) == 0u) return clus;
        }
        return 0u;
    };
    const auto ensure_dir = [&](const char name[11]) {
        if (has_entry(name)) return;
        uint32_t clus = alloc_cluster();
        if (clus < 2u) return;
        if (cluster_off(clus) + uint64_t(spc) * 512u > data.size()) return;
        ...
        /* FAT32 spec 1.03 section 6: the ".." entry of a directory whose
           parent is the root names cluster 0, not the root cluster. */
        write_short_entry(dir + 32u, "..         ", 0x10u, 0u);
        persist_range(cluster_off(clus), uint64_t(spc) * 512u);
    };
    const auto short_checksum = [](const char name[11]) -> uint8_t {
        uint8_t sum = 0;
        for (int i = 0; i < 11; ++i)
            sum = static_cast<uint8_t>(((sum & 1u) ? 0x80u : 0u) + (sum >> 1u) + static_cast<uint8_t>(name[i]));
        return sum;
    };
    const auto write_lfn_entry = [&](uint8_t* ent, const char* long_name, uint8_t checksum) {
    ...
    const auto ensure_root_file_min_size = [&](const char file_name[11], uint32_t min_size_bytes) {
        ...
        if (!zero_existing_chain(ent)) {
            ...
            for (uint32_t i = 0; i < needed_clusters; ++i) {
                ...
                if (clus < 2u) return;
                if (cluster_off(clus) + uint64_t(cluster_bytes) > data.size()) return;
                if (first == 0u) first = clus;
                if (prev != 0u) put_fat(prev, clus);
                prev = clus;
    ...
    const auto ensure_dir_file = [&](const char dir_short_name[11], const char file_name[11]) {
        ...
        for (uint32_t off = 0; off + 32u <= dir_bytes; off += 32u) {
            if (dir[off] == 0x00u) break;
            if (dir[off] == 0xE5u || ((dir[off + 11u] & 0x0Fu) == 0x0Fu)) continue;
            if (std::memcmp(dir + off, file_name, 11u) == 0) return;
        }
        for (uint32_t off = 0; off + 32u <= dir_bytes; off += 32u) {
    ...
    const auto ensure_fsinfo = [&]() {
        ...
        for (uint32_t clus = 2u; clus < total_clusters + 2u; ++clus) {
            if (get_fat(clus) != 0u) ++used_clusters;
        }
        const uint32_t free_count = total_clusters > used_clusters ? total_clusters - used_clusters : 0u;
    ...
    ensure_dir("ADDON      ");
    /* The panel ships \AddOn as a populated directory: the FSF volume inside
       the FWF update container is its content.  Materialize it once, on a
       directory this layout has just created and still owns. */
    if (uint8_t* addon_ent = find_short_entry("ADDON      ")) {
        const uint32_t addon_clus = cluster_from_entry(addon_ent);
        uint8_t* addon_dir = addon_clus >= 2u && cluster_off(addon_clus) + uint64_t(spc) * 512u <= data.size()
                                 ? data.data() + cluster_off(addon_clus)
                                 : nullptr;
        if (addon_dir && !addon_files.empty() && spc * 512u > 64u && addon_dir[64] == 0x00u) {
            uint32_t alloc_hint = 2u;
            fwf_fsf::FatSink fat;
            fat.cluster_bytes = spc * 512u;
            fat.alloc_cluster = [&]() -> uint32_t {
                const uint32_t total_clusters = static_cast<uint32_t>((data.size() - dataoff) / (uint64_t(spc) * 512u));
                for (; alloc_hint < total_clusters + 2u; ++alloc_hint) {
                    if (get_fat(alloc_hint) != 0u) continue;
                    if (cluster_off(alloc_hint) + uint64_t(spc) * 512u > data.size()) break;
                    put_fat(alloc_hint, 0x0FFFFFFFu);
                    return alloc_hint++;
                }
                return 0u;
            };
            fat.link_fat = [&](uint32_t from, uint32_t to) { put_fat(from, to); };
            fat.cluster_ptr = [&](uint32_t clus) -> uint8_t* {
                if (clus < 2u || cluster_off(clus) + uint64_t(spc) * 512u > data.size()) return nullptr;
                return data.data() + cluster_off(clus);
            };
            fat.persist = [&](uint32_t clus, uint32_t off, uint32_t len) {
                persist_range(cluster_off(clus) + off, len);
            };
            fwf_fsf::SeedFsfVolume(addon_files, root_clus, fat);
        }
    }
