...
public:
    ...
    bool ShouldRegister() override {
        return emu_.TryGet<Rtc8564Wiring>() != nullptr;
    }
...
private:
    ...
    void IncrementPointer() {
        pointer_ = static_cast<uint8_t>((pointer_ + 1u) & 0x0Fu);
    }

    uint8_t ReadRegister(uint8_t index) {
        index &= 0x0Fu;
        MaterializeTimer();
        if (!Stopped() && index >= kSeconds && index <= kYears)
            MaterializeClockRegisters();
    ...
    bool TimerEnabled() const {
        return (registers_[kTimerControl] & kTimerEnable) != 0 &&
               registers_[kTimer] != 0;
    }
    ...
    bool MaterializeTimer() {
        ...
        const uint64_t interval =
            static_cast<uint64_t>(timer_reload_) * TimerTickMicros();
    ...
    bool TimerInterruptEnabled() const {
        return (registers_[kControl2] & kTie) != 0;
    }

    bool RepeatedTimerMode() const {
        return (registers_[kControl2] & kTiTp) != 0;
    }

    bool LevelInterruptActive() const {
        const bool alarm = (registers_[kControl2] & (kAf | kAie)) ==
                           (kAf | kAie);
        const bool timer = !RepeatedTimerMode() &&
            (registers_[kControl2] & (kTf | kTie)) == (kTf | kTie);
        return alarm || timer;
    }

    uint64_t PulseRecoveryMicros() const {
        if (!RepeatedTimerMode())
            return 0;
    ...
    void WriteControl2(uint8_t value) {
        ...
        if ((requested & kTf) && (flags & kTf))
            next |= kTf;
        if ((requested & kAf) && (flags & kAf))
            next |= kAf;
    ...
    void MaterializeClockRegisters() {
        const std::time_t now = std::time(nullptr) +
            static_cast<std::time_t>(epoch_delta_seconds_);
    ...
    void CommitClockRegisters() {
        ...
        if (!BcdToBin(registers_[kSeconds] & 0x7Fu, 59, second) ||
            !BcdToBin(registers_[kMinutes] & 0x7Fu, 59, minute) ||
            !BcdToBin(registers_[kHours] & 0x3Fu, 23, hour) ||
            !BcdToBin(registers_[kDays] & 0x3Fu, 31, day) || day < 1 ||
            !BcdToBin(registers_[kMonths] & 0x1Fu, 12, month) || month < 1 ||
            !BcdToBin(registers_[kYears], 99, year)) {
            return;
        }
        std::tm target{};
        ...
        while (!timer_stop_.load(std::memory_order_acquire)) {
            {
                ...
                if (event && interrupt_delivery_armed_ &&
                    TimerInterruptEnabled() && RepeatedTimerMode()) {
    ...
    void UpdateInterrupt(uint64_t now) {
        if (interrupt_pulse_until_ != 0 && now >= interrupt_pulse_until_)
            interrupt_pulse_until_ = 0;
        SetInterrupt(interrupt_delivery_armed_ &&
            (interrupt_pulse_until_ != 0 || LevelInterruptActive()));
    }

    void SetInterrupt(bool active) {
        if (active == interrupt_asserted_)
            return;
