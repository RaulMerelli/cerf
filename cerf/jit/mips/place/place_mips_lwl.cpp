...
/* LWL rt, offset(rs): unaligned load-word-left. LwlHelper does the merge; the
   register index is passed (the helper reads and writes gpr[rt]). */
uint8_t* PlaceMipsLwl(uint8_t* cursor, MipsDecodedInsn* d, MipsBlockContext* ctx) {
    ...
    EmitCall(cursor, reinterpret_cast<void*>(&MipsMemoryAccess::LwlHelper));
    return cursor;
}
