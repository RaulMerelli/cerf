...
/* LBU rt, offset(rs): rt = zero_extend64(mem[gpr[rs] + sext(imm16)][7:0]).
   ...
   extended (rt.hi = 0). The load runs even when rt==0 (translate / fault / MMIO
   side effects are architectural); only the register write is skipped. */
uint8_t* PlaceMipsLbu(uint8_t* cursor, MipsDecodedInsn* d, MipsBlockContext* ctx) {
    ...
    EmitCall(cursor, reinterpret_cast<void*>(&MipsMemoryAccess::LoadByteHelper)); /* EAX = byte */
    if (d->rt != 0) {
