...
/* SD rt, offset(rs): mem[gpr[rs] + sext(imm16)] = gpr[rt] (full 64-bit). The
   register index is passed to StoreDwordHelper, which reads gpr[rt] itself; no
   r0 guard is needed because gpr[0] reads as 0. */
uint8_t* PlaceMipsSd(uint8_t* cursor, MipsDecodedInsn* d, MipsBlockContext* ctx) {
    ...
    EmitCall(cursor, reinterpret_cast<void*>(&MipsMemoryAccess::StoreDwordHelper));
    return cursor;
}
