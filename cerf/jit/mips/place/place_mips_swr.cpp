...
/* SWR rt, offset(rs): unaligned store-word-right. SwrHelper does the byte-wise
   store; the register index is passed (not the value). */
uint8_t* PlaceMipsSwr(uint8_t* cursor, MipsDecodedInsn* d, MipsBlockContext* ctx) {
    ...
    EmitCall(cursor, reinterpret_cast<void*>(&MipsMemoryAccess::SwrHelper));
    return cursor;
}
