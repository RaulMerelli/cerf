...
/* XORI rt, rs, imm : rt = rs ^ zero_extend(imm16), full 64-bit. The immediate
   ...
   copied even when rt != rs (skipping it would leave rt's stale high word). */
uint8_t* PlaceMipsXori(uint8_t* cursor, MipsDecodedInsn* d, MipsBlockContext*) {
    using namespace x86;
    if (d->rt == 0) {
        return cursor;
    }
    ...
    EmitXorRegReg(cursor, kEax, kEcx);
